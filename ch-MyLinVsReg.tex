...
In this Chapter, we guide the reader through the process of setting up a hierarchical Bayesian framework, establishing a choice of prior distributions, and using a DAG to visualise conditional dependencies between hyper-parameters and the parameter.
Applying the MTC scheme the marginal and then full conditional posterior distributions are explicitly formulated.

\mccorrect{
	The chosen Gaussian smoothness prior, together with the linear and later affine approximation of the forward model, provides the closest available Bayesian analogue of the smoothness-regularised retrieval framework employed in classical retrieval methods. This enables a direct comparison between the regularised and hierarchical Bayesian approaches.
}
...
A Metropolis within Gibbs (MWG) sampler and a TT approximation are utilised to characterise the marginal posterior.
...
For now the linear forward model matrix is defined as $\bm{A} \coloneqq \bm{A}_L$ by neglecting absorption in the RTE (see Chapter~\ref{ch:formodel}).
...
\begin{align}
	...
	\label{eq:GLapl} 
\end{align}
which is a discrete approximation to the second-order derivative operator with Dirichlet boundary condition and defines a 1-dimensional Graph Laplacian as in~\cite{wang2015graphs, fox2016fast}.
\mccorrect{This graph Laplacian introduces local dependence between neighbouring altitude layers and penalises large second-order differences in the profile. Consequently, the prior favours smooth ozone profiles while still allowing gradual changes with altitude. This reflects the physical expectation that atmospheric quantities vary smoothly over neighbouring atmospheric layers. 
%Furthermore, the resulting precision matrix is sparse and positive definite, making it computationally attractive for Bayesian inference.
	}
...
The corresponding prior ozone profiles according to $\bm{x}| \delta \sim \mathcal{N}(0, (\delta \bm{L})^{-1})$ are plotted in Fig.~\ref{fig:O3Prior}.
...
The regularisation parameter $\lambda$ will be properly introduced in the next section.
This advantageous when using the MWG algorithm, as in Sec.~\ref{subsec:MWG}, to sample from the marginal posterior distribution $\pi(\delta, \gamma | \bm{y})$.
\clearpage
...
The regularisation parameter $\lambda$ is introduced for better comparison of the regularisation approach (see Sec.~\ref{sec:SolByReg}) and hierarchical Bayesian framework.
...
\begin{align}
	f^{(r)}& (\lambda_0)= (-1)^{r+1} (\bm{A}^T \bm{y})^T (\bm{B}_0^{-1} \bm{L})^r \bm{B}_0^{-1} \bm{A}^T \bm{y} \label{eq:ftay} 
\end{align} 
...
We plot the function $f(\lambda)$ and $g(\lambda)$ and their approximations in Fig.~\ref{fig:fandg} and elaborate on the approximation errors further below.
$f(\lambda)$ is approximated with a 1st-order Taylor series.
...
\begin{algorithm}[!ht]
	...
	\begin{algorithmic}[1]
		\STATE Initialise \( ( \lambda^{(0)} , \gamma^{(0)}  ) \).
		\FOR{ \( k = 0, \dots, N-1 \)}
...
\begin{align} 
	...
	= \big( n/2+\alpha_{\delta}-1 \big) \Big(\log\lambda^{\prime} - \log\lambda^{(k)} \Big) - \frac{1}{2} \Delta g - \frac{\gamma^{(k)}}{2} \Delta f  + \beta_\delta \gamma^{(k)} \Delta \lambda  \, ,
\end{align}
...
\begin{mccorrection}
\noindent Since the proposal distribution is symmetric, the proposal terms in the Metropolis--Hastings acceptance ratio cancel. Using the 1st-order Taylor approximation of $f$ and the approximation of $g$ given in Eq.~\ref{eq:gAprox}, the log-acceptance probability simplifies to
\begin{align}
	\begin{split}
	\log\Bigg(  \frac{\pi(\lambda^{\prime} | \gamma^{(k)}, \bm{y})  }{\pi(\lambda^{(k)}| \gamma^{(k)}, \bm{y})}  \Bigg)& =\left(\log\lambda^{\prime} - \log\lambda^{(k)} \right)
	\left[ \left( n/2+\alpha_{\delta}-1 \right)- g^{\prime}(\lambda_0) \right] \\
	&\qquad - \Delta \lambda \, \gamma^{(k)}
	\left[\frac{f^{(1)}(\lambda_0)}{2}  + \beta_\delta \right],
	\end{split}
\end{align}
where $
	g^{\prime}(\lambda_0)
	\coloneqq 
	\frac{ g(1.25\lambda_{0}) - g(0.75\lambda_{0}) }
	{\log(1.25\lambda_{0}) - \log(0.75\lambda_{0}) } 
$~.
The error associated with this choice of approximation is discussed further  below.
\end{mccorrection} 


Next, a Gibbs step on $\pi(	\gamma|  \lambda^{(k+1)}, \bm{y}) $ conditioned on the previously drawn $ \lambda^{(k+1)}$ is performed.
Gibbs sampling is again a special case of the Metropolis-Hastings algorithm with acceptance probability equal to one.
Repeating this N times give us marginal posterior samples $(\lambda, \gamma)^{(1)}, \dots, (\lambda, \gamma)^{(N)} \sim  \pi(\lambda, \gamma| \bm{y})$.
To remove initialisation bias the first $N_{\text{burn-in}}$ samples are discarded.

Running the MWG sampler $f(\lambda)$ and $g(\lambda)$ are approximated around the mode $( \lambda_{0}, \gamma_0 )$ of $\pi(\lambda,\gamma| \bm{y})$ as previously described (see Fig.~\ref{fig:fandg}). 
$f(\lambda)$ is approximated with a 1st order Taylor series (see further below on errors due to those approximations).
...
This takes  $\approx 0.5$s.
The samples as well as the trace of the MWG algorithm to show ergodicity are plotted in Fig.~\ref{fig:ScatterPlotTT}.
...
\mccorrect{
	The hyper-parameter domain required for this TT approximation was selected using exploratory analysis and previous results obtained from the MWG sampler. The chosen intervals cover the region of significant posterior mass.
	The need to define a finite computational domain \emph{a priori} is a general limitation of the grid-based TT approximation.}
	
The univariate grid is defined over $\gamma = [ 0.8 \times 10^{15},1.2 \times 10^{16}]$ and $\lambda = [ 10^{-5}, 8 \times 10^{-4}]$ with $n_{\text{grid}} = 20$ grid points (see Fig.~\ref{fig:MeanVarError} and Sec.~\ref{subsec:firstCond} where we argue for the number of grid points).
...
%-320/2
A TT approximation is obtained via the~\texttt{tt.cross.rectcross.rect\_cross.cross} Python function, which implements the TT cross algorithm from~\cite{OSELEDETS2010TTCross,Dolgov2018TTCross}, in the Python package \texttt{ttpy}~\cite{Oseledets2018ttpy}. The Python function is initialised with a random TT, which has a constant number of ranks $r = 4$.
Note that the ranks are kept constant and are not increased within this process.
One sweep within the \texttt{ttpy.$(\cdots)$.cross} Python function results in $2n_{\text{sweeps}}2n_{\text{grid}} =400$ function evaluations and takes $\approx 0.02$s.
%Ironically,
This is the same number of function evaluations to approximate a $20 \times 20$ point grid.
The TT format is especially advantageous for larger grid sizes and higher-dimensional parameter spaces.



To compute the marginals $\pi(\lambda| \bm{y})$ and $\pi(\gamma| \bm{y})$ the constant in Eq.~\ref{eq:SQTTapprox} is set to $\xi = 1 / \uplambda (\mathcal{X})$ because we observe unormalised marginal posterior values of around $10^{47}$.
...
The relative RMS TT approximation error over the whole grid is $\approx 7\%$ and similar to the propagation error in $\pi(\lambda, \gamma| \bm{y})$ due to the approximations of $f(\lambda)$ and $g(\lambda)$ (see below).
...
Compared to a 2nd-, 3rd- or 4th-order Taylor approximation, the 1st-order Taylor approximation of $f(\lambda)$ gives the smallest relative RMS error of $\approx 9 \%$ for $\lambda = [ 10^{-5}, 8 \times 10^{-4}]$ (TT grid) and a maximum absolute error of $\approx 3 \times 10^{-16}$.
...
\subsection{Full Conditional for Ozone}
\label{subsec:firstCond}

Given the marginal posterior over the hyper-parameters, we can evaluate the normally distributed full conditional for ozone
\begin{align}
...
\begin{figure}[ht!]
	\centering
	\includegraphics{My_FirstTestRes.png}
	\caption[Ozone samples of the full posterior.]{50 ozone samples from the full conditional for ozone, posterior mean and covariance are characterised by weighted expectations over the marginal posterior $\pi(\lambda,\gamma | \bm{y})$ based on the linear forward map $\bm{A}_L$. We set negative ozone VMR values to zero.}
	\label{fig:O3Samp}
\end{figure}
In this thesis, the posterior ozone mean
\begin{align}
...
\end{align} and posterior ozone covariance
\begin{align}
	\bm{\Sigma}_{\bm{x}|\bm{y}} &= \int \gamma^{-1}  \pi(\gamma | \bm{y} ) \, \text{d}\gamma  \int  \bm{B}_{\lambda}^{-1} \pi(\lambda | \bm{y} )  \, \text{d} \lambda + \int ( \bm{x}_{\lambda} - \bm{\mu}_{\bm{x}|\bm{y}})( \bm{x}_{\lambda} - \bm{\mu}_{\bm{x}|\bm{y}})^T  \pi(\lambda | \bm{y} )  \, \text{d} \lambda   \\ &\approx \sum {\gamma_i}^{-1}\pi(\gamma_i| \bm{y}) \sum \bm{B}_{\lambda_i}^{-1}\pi(\lambda_i| \bm{y}) + \sum ( \bm{x}_{\lambda_i} - \bm{\mu}_{\bm{x}|\bm{y}})( \bm{x}_{\lambda_i} - \bm{\mu}_{\bm{x}|\bm{y}})^T \pi(\lambda_i | \bm{y} )  \, \label{eq:CovInt}
\end{align}
 are computed as weighted expectations over the marginal posterior $\pi(\lambda,\gamma | \bm{y})$ by quadrature \cite[Sec. 2.1]{Dick_Kuo_Sloan_2013} with $\sum \pi(\lambda_i| \bm{y}) = \sum \pi(\gamma_i| \bm{y}) = 1$.
...
More precisely, the heights of the histogram bars act as quadrature weights, where $\lambda_i$ is defined at the centre of each bar.
...
It is sufficient to evaluate $\bm{x}_{\lambda}$ and invert $\bm{B}_{\lambda}$ 20 times to obtain posterior ozone mean $	\bm{\mu}_{\bm{x}|\bm{y}} $ and covariance values $	\bm{\Sigma}_{\bm{x}|\bm{y}} $ within a reasonable error (see Fig.~\ref{fig:MeanVarError}).

If calculating the posterior mean and covariance matrix is too costly, see Sec.~\ref{subsec:RTO} where we present a feasible alternative to draw a sample from the full conditional posterior.
Posterior samples are plotted in Fig.~\ref{fig:O3Samp} with negative ozone values set to zero.
...
Finding the mode of $\pi(\lambda,\gamma|\bm{y})$, running the TT cross algorithm, calculating the marginals and the posterior mean and covariance takes $0.025$s.
Using the MWG sampler takes $\approx0.5$s for the same results, so most computational effort lays within the sampling procedure.
...
Here, we provide an average error estimate for the posterior mean $\bm{\mu}_{\bm{x}|\bm{y}}$ and the covariance matrix $\bm{\Sigma}_{\bm{x}|\bm{y}}$ due to the TT grid size of the marginal posterior or the number of histogram bins of the marginal posterior samples.

In doing so, we bin the samples from $\pi(\lambda,\gamma | \bm{y})$ into a 200 bin histogram and calculate the ``true'' posterior mean $\bm{\mu}^{(200)}_{\bm{x}|\bm{y}}$ and the ``true''  covariance $\bm{\Sigma}^{(200)}_{\bm{x}|\bm{y}}$ by weighted expectations, as in Eq.~\ref{eq:MeanInt} and Eq.~\ref{eq:CovInt}.
Then, the posterior mean $\bm{\mu}^{(n_{\text{bin}})}_{\bm{x}|\bm{y}}$ and the covariance $\bm{\Sigma}^{(n_{\text{bin}})}_{\bm{x}|\bm{y}}$ for various number of bins $n_{\text{bin}}$ are calculated.
The relative RMS error for each estimate is
\begin{align} 
	\frac{\bigg\lVert \text{vec}\bigg(\bm{\mu}^{(n_{\text{bin}})}_{\bm{x}|\bm{y}},\bm{\Sigma}^{(n_{\text{bin}})}_{\bm{x}|\bm{y}}\bigg) - \text{vec}\bigg(\bm{\mu}^{(200)}_{\bm{x}|\bm{y}},\bm{\Sigma}^{(200)}_{\bm{x}|\bm{y}}\bigg) \bigg\rVert_{L_2}  }{\bigg\lVert \text{vec}(\bm{\mu}^{(200)}_{\bm{x}|\bm{y}},\bm{\Sigma}^{(200)}_{\bm{x}|\bm{y}}\bigg)\bigg \rVert_{L_2} } \, ,
\end{align} 
where $\text{vec}\bigg(\bm{\mu}^{(n_{\text{bin}})}_{\bm{x}|\bm{y}},\bm{\Sigma}^{(n_{\text{bin}})}_{\bm{x}|\bm{y}}\bigg)$ denotes a vector with $n + n \times n$ entries, for $n = 34$.

In Fig. \ref{fig:MeanVarError}, the relative RMS error of the posterior mean and covariance for an increasing number of bins is plotted.
The relative RMS error decreases roughly proportional to $1/N$, and we consider a relative RMS error of less than $0.5\%$ good enough.
This is easily met at 20 bins.
...
\begin{figure}[ht!]
	\centering
	\includegraphics{My_relErrO3MeanVar.png}
...
\subsubsection{Eigenvalues of the full conditional posterior covariance matrix}
To understand how the prior influences samples from the full conditional posterior, we examine the eigenvalues of the full conditional precision matrix $\bm{Q}_{\bm{x} \mid \delta, \gamma, \bm{y}}$.
In Fig.~\ref{fig:PostCov}, the eigenvalues (ordered in size) of $\bm{Q}_{ \bm{x}|\delta, \gamma,\bm{y}}=  \gamma \bm{A}^T \bm{A} + \delta \bm{L} $ for a random $\delta,\gamma \sim \pi(\delta,\gamma|\bm{y})$ are plotted and compared to the eigenvalues of $\gamma \bm{A}^T \bm{A}$ and the prior precision matrix $\delta \bm{L}$.
We observe that the larger eigenvalues of $\bm{Q}_{ \bm{x}|\delta, \gamma,\bm{y}}$ and $\gamma \bm{A}^T \bm{A}$ are very much the same.
...
\begin{figure}[ht!]
	...
	\caption[Eigenvalues of the posterior precision matrix]{Eigenvalues of the precision matrix of $\bm{Q}_{ \bm{x}|\delta, \gamma, \bm{y}}= \gamma \bm{A}^T \bm{A} + \delta \bm{L}$ of the full conditional posterior distribution $\pi(\bm{x}|\delta, \gamma, \bm{y})$ for ozone.
		Large eigenvalues of $\bm{Q}_{ \bm{x}|\delta, \gamma, \bm{y}} $ are unaffected by the prior.
...
A regularised solution is defined as in~\cite{hansen2010discrete, fox2016fast} 
\begin{align}
	\bm{x}_{\lambda} =\underset{ \bm{x}}{\arg \min}\,  \lVert \bm{A}\bm{x} - \bm{y} \rVert_{L^2}^2 + \lambda \bm{x}^T \bm{L} \bm{x} \, ,
	\label{eq:XLam}
\end{align}
with the regularisation parameter $\lambda$, linear forward model matrix $\bm{A}$, data $\bm{y}$ and the regularisation matrix $\bm{L}$ as in Eq.~\ref{eq:GLapl}.
Solving Eq.~\ref{eq:XLam} for $\bm{x}_{\lambda}$ as in Sec.~\ref{sec:reg} gives the regularised solution
\begin{align}
...
The regularised estimate corresponds to the ``corner'' of the L-curve at the point of maximum curvature.
This is provided by the kneedle algorithm~\cite{satopaa2011kneedle} using the function \texttt{kneed.KneeLocator} in $\approx 0.015$s.
The MTC scheme with the TT approximation of the marginal posterior to obtain posterior mean and covariance via weighted expectations is slightly slower ($\approx 0.025$s).
The corresponding regularisation parameter is $\lambda = 1.6 \times 10^{-4}$.

\begin{figure}[ht!]
	\centering
	\includegraphics{LCurvePhD.png}
	\caption[Plot of the L-curve to find the regularised solution.]{L-curve of regularisation norm $\sqrt{\bm{x}^T\bm{Lx}}$ against the data misfit norm $\lVert \bm{A}\bm{x} - \bm{y} \rVert_{L^2}$ for different $\lambda$ values, where $\bm{x}_{\lambda}$ is calculated as in Eq.~\ref{eq:xLam}. The regularised estimate is located at the point of maximum curvature (pink triangle) of the L-curve. Additionally, we calculate the data misfit norm and the regularisation norm for the mean (black circle) and samples (blue squares) of the full posterior of ozone.}
	\label{fig:LCurve}
\end{figure}
...
The data misfit and regularisation norm values of the regularised estimate and the posterior mean are not similar to the corresponding values of the samples drawn from the full conditional for ozone $\pi(\bm{x}| \delta, \gamma,\bm{y})$ (see Fig.~\ref{fig:O3Samp}).
All posterior ozone samples are less regularised and lie above the L-curve, and as already mentioned, are all feasible solutions to the data.
...
Note that we cannot infer the full posterior mean from the regularised solutions, even if it appears to lie on the L-curve.

%Counterintuitively, it is not possible to obtain a sample that exaclty represents the mean of a multivariate probability distribution~\cite[Sec.~3.1]{VershyninMultiD2018}.
Counterintuitively, it is not possible to obtain a sample that exactly represents the mean of a multivariate probability distribution~\cite[Sec.~3.1]{VershyninMultiD2018}. 
This is because the probability of drawing any specific point (point estimate) is zero for continuous distributions.
%, despite it being the point of the highest probability.
%Further the probability mass in a multi-dimensional space concentrates on a “shell” or “ring” away from the mean~\cite{VowelsMultiD}.
Furthermore, in high-dimensional spaces the probability mass often concentrates on a “shell” or “ring” around the mean rather than at the mean itself~\cite{VowelsMultiD}. As a result, typical samples are located some distance away from the mean, even though the mean may correspond to the most probable point.
