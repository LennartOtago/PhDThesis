...
\mccorrect{The contribution of this thesis is not to propose a new atmospheric retrieval model, but rather to extend the retrieval framework to account for uncertain temperature and pressure profiles directly within the inference process. Instead of conditioning on external point estimates, temperature and pressure are treated as uncertain quantities whose variability and dependence on the ozone profile are explicitly represented. This enables the joint estimation of atmospheric quantities together with their uncertainties and allows dependencies, correlations, and competing effects between key model parameters to be quantified. Consequently, the framework provides a more complete characterisation of retrieval uncertainty than approaches based solely on point estimates and ad-hoc error propagtion.
In particular, the framework makes it possible to investigate how uncertainties in temperature and pressure propagate into ozone retrievals and to identify the dominant sources of uncertainty.
}
