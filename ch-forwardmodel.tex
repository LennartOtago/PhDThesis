...
A noise-free measurement vector is given as $\bm{A}_L \bm{x}$.
...
The SVD gives us information on how information on $\bm{A}_L$ acts on the parameter through the right singular vectors $\bm{v}_i$.
...
And again for large singular values above the SNR we expect reconstructions to be close to the ground truth as the data is hardly influenced by the noise.
...
Case 3, case 4, and case 5 measure at equidistance spaced pointing angles corresponding to different pointing accuracies.
The pointing accuracy determines how well the satellite can point in a certain direction and, hence the smallest tangent height difference in a data set.
Case 1, case 2, case 3, and case 4 measure in between heights $h_{L,1} = 6.9$km and $h_{L,n} = 83.3$km, case 5 and does not collect measurements in high altitude regions.
...
We visually asses the effective rank and how the singular values behave to determine which of the test cases is most effective.
More specifically, if the singular values decay fast and only few singular values are above a SNR of 150 the forward map is rather uninformative.
If the singular values decay less fast and more singular values are above a SNR of 150 we classify the forward map as informative.
...
\begin{figure}[ht!]
	...
	\label{fig:SingA}
\end{figure}
In Fig.~\ref{fig:SingA} we plot the singular values for each of the measurement cases and the SNR corresponding to the singular values of case 5.
The dotted SNR lines in Fig.~\ref{fig:SingA} correspond a SNR of roughly 150 and the SNR according to the lowest singular values which would be required to reconstruct all information provided by the forward model.
The largest singular values of case 5 is roughly equal to the largest singular values of case 1, case 3, and case 4, \textcolor{red}{so the SNR would be in a similar region.}
Fig.~\ref{fig:SingA} shows that case 2 has the largest singular value of all cases but its singular values decreases faster in the region where noise is becoming dominant than the singular values of cases 3, case 4, and case 5.
In contrast case 1 does obtain singular values which are decreasing the fastest of all cases and the most number of singular values below a SNR of 150.
Additionally case 1 and case 2 have a large number of very low singular values and, so neither of those cases are optimal.

The singular values of the cases 3, 4, and 5 with equidistance spaced pointing angles behave similar.
...
The slightly larger pointing accuracy in case 5 compared to case 3 provide similar information.
Since the last 5 to 10 singular values of case 3 are so small that the information will be completely covered by the noise.
Hence we do not measure in noise dominated regions and case 5 measures only up to height of $\approx 68$km instead of $\approx83$km without losing too much crucial information above the SNR.
...
The test cases show that a suitable measurement strategy may consist of equidistance spaced pointing angles and does stop measuring when the singular values (information) are too low. 
...
This is roughly where the noise starts to dominate the data and the space spanned by those right singular vectors represent parameter values the middle atmosphere.
Consequently we expect a increasing uncertainty of reconstructed parameter values within the middle of the atmosphere of $\approx20$km to $\approx55$km.
The singular vectors in Fig.~\ref{fig:nullSpace} corresponding to the smallest 10 singular values span parameter structures in higher altitudes.
Since the singular values are very small we will not be able reconstruct structures above $\approx55$km and the data is noise-dominated.
...
As already mentioned a SNR of 150.
