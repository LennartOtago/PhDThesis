...
\end{align}
is dependent on the line intensity $L(\nu, T_{\text{ref}})$ at reference temperature $T_{\text{ref}} =296K $, the lower-state transition energy $ E^{\prime \prime} $, the second radiation constant $c2=1.4387769\text{cmK}$ all provided by the HITRAN database \cite{gordon2022hitran2020}.
...
\begin{figure}[ht!]
	...
	\caption[Last 10 right singular vectors of forward model.]{We plot the last 10 right singular vectors of the forward model matrix for the case 5 sequence of measurements, as displayed in Fig.~\ref{fig:TangHCases}. These singular vectors correspond to small singular values of the forward model, see Fig.~\ref{fig:SingA}.}
	\label{fig:nullSpace}
\end{figure}
