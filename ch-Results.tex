...
Ironically, this the same number of functions evaluations to approximate a 20 point grid in 2D, where the marginalisation over $\lambda$ or $\gamma$ in the TT format is computationally less expensive compared to a classical marginalisation over each individual hyper-parameter value.
For larger grid sizes and higher dimensional parameter spaces the TT format is much more advantageous.
To compute the marginals $\pi(\lambda| \bm{y})$ and $\pi(\gamma| \bm{y})$ we set $\xi = 1 / \uplambda (\mathcal{X})$ and $\uplambda(x) = 1$, so that for Cartesian basis $\bm{M}_k = \text{diag}(\uplambda_k(\mathcal{X}_k))$ in Eq.~\ref{eq:MassMat}.
...
\begin{figure}[htb!]
	...
	\end{tikzpicture}
	\caption[Strategy to find affine map.]{The strategy to find the affine map consists of evaluating the marginal posterior for ozone $\pi(\lambda , \gamma  | \bm{y})$ based on the linear forward model first. Then we draw ozone samples from the full posterior. Next, we find an affine map which approximates between noise-free data based on the linear and non-linear forward model.}
...
Again, we use the MTC scheme and the exact same setup and procedure as in Sec. \ref{sec:FirstO3Post} to evaluate the marginal posterior and then the conditional posterior of ozone with similar computational time.
...
As in Sec. \ref{sec:BayModelO3}, we use the DAG in Fig.~\ref{fig:DAGComplete} to visualise the measurement process and correlations between parameters.
...
Here, we define the marginal and then the conditional posterior distribution for the described Bayesian model.
We either use the \texttt{t-walk} algorithm~\cite{christen2010general} to draw samples from $\pi(p_0,b,T_0,\bm{h_T},\bm{a} ,\delta, \gamma| \bm{y})$ or we utilise a TT approximation on a predefined grid to generate samples via the SIRT method with an MH correction step.
...
Lastly, we use the RTO method to draw samples from the conditional posterior $\pi(\bm{x}|p_0,b,T_0,\bm{h_T},\bm{a} ,\delta, \gamma, \bm{y})$.
...
For a ground truth, we run the \texttt{t-walk}~\cite{christen2010general} algorithm on $\pi( \bm{\theta}_{\bm{p}, \bm{T}},\lambda,\gamma  | \bm{y})$, where our objective is to generate $\approx 1000$ independent samples from the marginal posterior.
...
We initialise the Python implementation of the \texttt{t-walk}~\cite{christentwalkaccess} around the hyper-prior mean values and take a total number of $N' =N + N_{\text{burn-in}} = 1210000$ steps around $10$ mins within bounds given by the iteratively defined TT grid (see Tab.~\ref{tab:priors}).
...
We set the number of grid points to $n = 150$ and calculate different error measures for deceasing number of ranks to find the optimal number of ranks, where we compare to $1000$ independent \texttt{t-walk} samples from the true marginal posterior.
...
\end{figure}
For stable and comparable results, we do 5 sweeps in \linebreak \texttt{tt.cross.rectcross.rect\_cross.cross} initialised with a random TT.
Then we draw $1000$ independent samples from the TT approximation of the marginal posterior via the SIRT-MH scheme. 
To calculate the 1-Wasserstein distance, as in Eq. \ref{eq:applWasser}, between SIRT-MH samples weighted with the TT approximation of marginal posterior, and the \texttt{t-walk} samples, weighted by the true marginal posterior value, we use the \texttt{SamplesLoss("sinkhorn", p=1, blur=0.05, scaling=0.8)} function with default settings from the Python package \texttt{geomloss}~\cite{Wassersteinaccess}.
...
Then we decrease the grid size and decide that for $n \geq 20$ the relative differences of sample-based means to $\bm{\mu}_{\texttt{t-walk}}$ (red circles in Fig.~\ref{fig:FindRankGrid}) and the RMS error at the SIRT-MH samples (orange squares in Fig.~\ref{fig:FindRankGrid}) around $10 \% - 20\%$ is good enough.
...
Further, we decrease the number of functions evaluation and define ranks \linebreak$r = [ 1,  10,  10, 10, 10, 10, 5, 5, 5, 5, 5, 3, 2, 2, 2, 2, 2, 2, 1]$ harvesting the correlation structure of $\bm{\theta}_{\bm{p}, \bm{T}},\lambda,\gamma  | \bm{y})$ even more.
We do one sweep in \linebreak \texttt{tt.cross.rectcross.rect\_cross.cross}, reducing the computation time to $\leq5$s and the number of function evaluations to 24120, where we initialise at a previously calculated approximation.
...
We use the RTO method (see Sec. \ref{subsec:RTO}) to obtain ozone samples from the conditional posterior and sample hyper-parameter samples directly from the marginal posterior to compute posterior temperature and pressure profiles according to their respective function (see Eq.~\ref{eq:tempFunc} and Eq.~\ref{eq:pressFunc}). 
...
Since the hyper-parameter $b$ is smaller than its ground truth value, the posterior pressure profile does not exponentially decrease as strong as the ground truth pressure.
