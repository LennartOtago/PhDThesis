...
We established a choice of prior distributions within our Bayesian model and formulated the posterior distributions.
...
Then we calculate the mean and the covariance matrix of the full posterior for ozone, which we use to find an affine map to approximate the non-linear forward model $\bm{A}_{NL} \approx \bm{M} \bm{A}_{L} \bm{x}$ (see Sec.~\ref{sec:affineMap}).
...
Then we can compute a data vector $\bm{y} = \bm{A}_{NL} + \bm{\eta} $, with $m = 30$ measurements determined by the satellite pointing accuracy of $175\text{arc sec}$ (see case 5 in Fig.~\ref{fig:TangHCases}), according to the RTE as in Eq.~\ref{eq:RTE} and~\ref{eq:absRTE} using the trapezoidal integration rule.
...
Lastly, we add independently normally distributed $\bm{\eta} \sim \mathcal{N}(0,\gamma^{-1} \bm{I})$ noise so that the $\text{SNR}=150$ (see Eq.~\ref{eq:SNR}) similar to~\cite{Froidevaux2008snrozone}, where a signal with a maximal spectral intensity of around $100\text{K}$ and a noise range of $0.4$ to $1.6\text{K}$ is reported.
...
We can visualise this hierarchical structure and the correlations between different hyper-parameters and parameters through a DAG.
...
In this case, $\theta_{\gamma}$ determines the shape and rate of the hyper-prior distribution $\pi(\gamma)$.
Similarly $\theta_{\delta}$ defines $\pi(\delta)$, which then deterministically sets the prior precision $\bm{Q}(\delta)$, where $\delta$ accounts for smoothness of the ozone profile.
...
By choosing a normally distributed prior $\pi(\bm{x}|\delta)$ with zero mean and no other restrictions, it is clear that our model does not take into account that ozone values can not be negative.
...
\begin{figure}[th!]
	...
	\caption[Plot of the functions $f(\lambda)$ and $g(\lambda)$ for marginal posterior.]{Plot of the functions $f(\lambda)$ and $g(\lambda)$ from the marginal posterior for a wide range of $\lambda = \delta / \gamma$. We plot the approximations (see Eq.~\ref{eq:fAprox} and \ref{eq:gAprox}) in black around the mode of the marginal posterior (vertical line) for the sampling range of $\lambda$ within the MWG algorithm.}
	\label{fig:fandg}
\end{figure}
For each evaluation of the marginal posterior most of the computational effort flies in the calculation of $f(\lambda)$ and $g(\lambda)$.
...
We initialised the MWG algorithm at the mode $(\lambda^{(0)} , \gamma^{(0)}  ) = ( \lambda_{0} , \gamma_{0}  )$ and take $N = 10000$ plus $N_{\text{burn-in}} = 100$ steps in approximately $0.3$s.
...
\begin{figure}[h!]
	...
	\caption[IATC of $\lambda$ samples from $\pi(\gamma, \lambda| \bm{y})$, for linear model.]{Provided by \cite{drikHesse}, the autocorrelation function $\Gamma_{\lambda}$ at different lags W is plotted as well as the IATC $\tau_{\text{int},\lambda}$ for the samples from $\pi(\gamma, \lambda| \bm{y})$ based on the linear forward model.}
...
The number of ranks is set to constant $r = 5$, we do 1 sweep with $2n_{\text{sweeps}}((d-2)r^2n+ 2nr) =400$ function evaluations and obtain a TT approximation of $\pi( \lambda,\gamma| \bm{y})$ in about $0.1s$.
Next, we compute the marginals $\pi(\lambda| \bm{y})$ and $\pi(\gamma| \bm{y})$ as described in Sec.~\ref{subsec:TTMarg}.
In doing so, we set $\xi = 1 / \uplambda (\mathcal{X})$ and $\uplambda(x) = 1$, so that for Cartesian basis $\bm{M}_k = \text{diag}(\uplambda_k(\mathcal{X}_k))$ in Eq.~\ref{eq:MassMat} and calculate the coefficient tensor $\bm{B}$ and $\bm{R}_{\text{pre}}$ as in Prop.~\ref{prob:backMarg} and Prop.~\ref{prob:ForMarg}.

We plot the TT approximation as a colour map on top of the obtained samples in Fig.~\ref{fig:ScatterPlotTT}.
The relative RMS error over the whole grid of $7\%$, similar to the propagation error in $\pi(\lambda, \gamma| \bm{y})$ due to approximations of $f(\lambda)$ and $g(\lambda)$ (see further up).
\begin{figure}[h!]
	\centering
	\includegraphics{ScatterplusHistoPlusTT.png}
	\caption[Samples from marginal posterior and TT approximation; trace plot of the MWG for $\pi(\lambda, \gamma| \bm{y})$]{We scatter plot the samples of $\lambda = \delta / \gamma $ and $\gamma$ from the marginal posterior $\pi(\lambda , \gamma  | \bm{y})$ and colour code the samples using the TT approximation of $\pi(\lambda , \gamma  | \bm{y})$. The mode of $(\lambda_0 , \gamma_0)$ of $\pi(\lambda , \gamma  | \bm{y})$ is marked by the pink cross. To show ergodicity, we plot the trace of the samples of the MWG algorithm below.}
...
More precisely, the height of the sample-based histogram bars acts as quadrature weights, where $\pi(\lambda_i| \bm{y})$ is defined at the centre of each bin.
...
This indicates that we should use a different, more physically based prior or model to parametrise the ozone profile.
...
\paragraph{Errors of Posterior Mean and Covariance}
\begin{figure}[ht!]
	\centering
	\includegraphics{relErrO3MeanVar.png}
	\caption[Relative Error of full posterior mean and covariance.]{Here, we plot the relative RMS error of the weighted expectations of $\bm{\mu}_{\bm{x}|\bm{y}}$ and covariance $\bm{\Sigma}_{\bm{x}|\bm{y}}$ compared to a ``ground truth'' given by weighted expectations over 200 bins.}
...
Here we write $\bm{A}_{NL}(\bm{x})~\coloneqq~\bm{A}(\bm{x})$ for non-linear noise-free data and $\bm{A}_L\bm{x}$ for linear noise-free data, both with ground truth pressure and temperature.
\begin{figure}[htb!]
	\centering
	\begin{tikzpicture}
	...
	\end{tikzpicture}
	\caption[Strategy to find affine map.]{The strategy to find the affine map consists of evaluating the marginal posterior for ozone using the linear forward model. Then we draw ozone samples from the conditional posterior and calculate noise-free data based on the linear and non-linear forward model. Next, we find a mapping between those two spaces so that we can approximate the non-linear forward model using an affine map and the linear forward model.}
...
We assess the affine map by calculating the relative RMS difference $\lVert \text{vec}(\bm{M}\bm{W}) - \text{vec}(\bm{V})  \rVert_{L^2} / \lVert \text{vec}(\bm{M}\bm{W}) \rVert_{L^2} $, the mapped linear noise-free data and the non-linear noise-free data, which is approximately $0.001\%$.
\begin{figure}[t!]
	\centering
	%\includegraphics{SampMapAssesment.png}
	\includegraphics{SampMapAssesmentTT.png}
	\caption[Assessment of affine map.]{We assess how well we can map a new ozone sample $\bm{x} \sim \pi(\bm{x}|\bm{y})$ from the linear forward model onto the non-linear forward model using the previously calculated affine map $\bm{M}$. The here used sample has not been used to create this affine map. The grey stars represent linear noise-free data, whereas the red circles represent non-linear noise-free data. The approximated non-linear noise-free data, black stars, has a RMS error of $\approx 0.03\%$ compared to the non-linear noise-free data.}
	\label{fig:MapAsses}
\end{figure}
...
Again, we use the MTC scheme and the exact same setup and procedure as in Sec. \ref{sec:FirstO3Post} to evaluate the marginal posterior and then the conditional posterior of ozone.

\begin{figure}[ht!]
	\centering
	\includegraphics{secMargO3Res.png}
	\caption[Marginal posterior histograms and TT approximation as well as hyper-prior distribution.]{We plot the TT approximation of the marginal posterior in orange and the samples as a histogram, as well as the prior distribution with a dotted line. Note that we sample $\lambda$ and $\gamma$ using the MWG algorithm and then can calculate $\delta$ for every sample of the marginal posterior. The regularised parameter corresponding to the best regularised solution (see Fig.~\ref{fig:O3SolplsReg} and Fig.~\ref{fig:LCurve}) is marked through the red vertical line. We mark the ground truth noise precision through the back vertical line.}
...
We initialise the MWG at the mode of $\pi(\lambda,\gamma| \bm{y})$ and approximate $f(\lambda)$ and $g(\lambda)$ around the mode as in Eq.~\ref{eq:fAprox} and Eq.~\ref{eq:gAprox} and take $N = 10000$ plus $N_{\text{burn-in}} = 100$ steps.
We plot the samples in Fig. \ref{fig:MargPostHistTT} as well as the marginal approximations provided by the TT decomposition on the same grid as previously defined with the same number of ranks (see Sec.~\ref{subsec:FirstMargPost}).
...
We also note that compared to the previously calculated mean and variance based on the linear forward model $\bm{A}_L$ (see \ref{fig:O3Samp}), the posterior distribution based on $\bm{M A}_L$ does not differ significantly.
...
%	\caption[Eigenvalues of the posterior precision matrix]{Eigenvalues of the precision matrix of $\bm{Q}_{ \bm{x}|\bm{y}}= \big(\int \gamma^{-1}(\bm{A}^T \bm{A} + \lambda \bm{L})^{-1}\diff \pi(\lambda ,\gamma)\big)^{-1}$ of the full posterior distribution $\pi(\bm{x}|\bm{y})$ for ozone. %Since $\bm{Q}_{ \bm{x}|\bm{y}} $ is symmetric and positive definite the singular values are equal the absolute value of the eigenvalues, $\sigma_i = |\uplambda_i|$.
%		%The square root of the eigenvalues of $\bm{A}^T\bm{A}$ are the singular values of $\bm{A}$; $\sigma_i =\sqrt{\uplambda_i}$.
%		We see that large eigenvalues of $\bm{A}^T \bm{A}$ are rather unaffected by the prior compared to small eigenvalues.}
%	\label{fig:PostCov}
%\end{figure}
%Additionally in Fig.~\ref{fig:PostCov}, we plot the eigenvalues of the precision matrix $\bm{Q}_{ \bm{x}|\bm{y}}= \big(\int \gamma^{-1}(\bm{A}^T \bm{A} + \lambda \bm{L})^{-1}\diff \pi(\lambda ,\gamma)\big)^{-1} $ and compare to the eigenvalues of $\bm{A}^T \bm{A}$ to visualise how the data and the prior effect the posterior ozone profiles.
...
We plot the corresponding regularisation parameter in Fig. \ref{fig:MargPostHistTT}, which is significantly larger than the $\lambda$-samples of the marginal posterior.
\begin{figure}[ht!]
	\centering
	\includegraphics{LCurvePhD.png}
	\caption[Plot of the L-curve to find the regularised solution.]{We calculate regularised solution as in Eq.~\ref{eq:xLam} and plot the regularised semi norm $\sqrt{\bm{x}^T\bm{Lx}}$ against the data misfit norm $\lVert \bm{A}\bm{x}_\lambda - \bm{x} \rVert_{L^2}$ to find the regularised solution at the point of maximum curvature of the so-called L-Curve. Additionally, we calculate the data misfit norm and the regularised norm for the ozone posterior and for samples of the conditional posterior distribution.}
	\label{fig:LCurve}
\end{figure}
...
\begin{subequations}
	\label{eq:fandg}
	\begin{align}
		&f ( \lambda,\bm{\theta}_{\bm{p}, \bm{T}}) = \bm{y}^T \bm{y} - \big(\bm{A}(\bm{\theta}_{\bm{p}, \bm{T}})^T \bm{y}\big)^T \big(\bm{A}(\bm{\theta}_{\bm{p}, \bm{T}})^T  \bm{A}(\bm{\theta}_{\bm{p}, \bm{T}}) + \lambda \bm{L}\big)^{-1} \big(\bm{A}(\bm{\theta}_{\bm{p}, \bm{T}})^T \bm{y}\big)  \label{eq:fFullAppl} \, ,  \\
		&\text{and } g(\lambda,\bm{\theta}_{\bm{p}, \bm{T}}) = \log \det \big(\bm{A}(\bm{\theta}_{\bm{p}, \bm{T}})^T  \bm{A}(\bm{\theta}_{\bm{p}, \bm{T}}) + \lambda \bm{L}\big) \label{eq:gFullAppl} \, .
	\end{align}
\end{subequations}
...
Additionally we use the marginal functions of each TT approximation to calculate the weighted means $\bm{\mu}_{\text{TT}} \in \mathbb{R}^{18}$ of each hyper-parameter and then the relative RMS difference $\lVert\bm{\mu}_{\text{TT}} - \bm{\mu}_{\texttt{t-walk}} \rVert_{L^2} / \lVert \bm{\mu}_{\texttt{t-walk}} \rVert_{L^2}  $ compared to "true means"  $\bm{\mu}_{\texttt{t-walk}}$ from the \texttt{t-walk}.
...
To decrease the number of functions evaluation and define ranks \linebreak$r = [ 1,  10,  10, 10, 10, 10, 5, 5, 5, 5, 5, 3, 2, 2, 2, 2, 2, 2, 1]$ according to the correlation structure of $\pi( \lambda,\bm{\theta}_{\bm{p}, \bm{T}},\gamma  | \bm{y})$ (see Fig. \ref{fig:CorrPlot}).
...
We plot the posterior temperature and pressure profiles in Fig.~\ref{fig:TempPost} and Fig.~\ref{fig:PressPost} and posterior ozone in Fig.~\ref{fig:O3Post}.
\begin{figure}[ht!]
	\centering
	\includegraphics{TempPostMeanSigm.png} 
	\caption[Temperature posterior samples.]{We take samples from the posterior distribution, as plotted in Fig.~\ref{fig:PostHistTT0} to Fig.~\ref{fig:PostHistTT3} and plot the corresponding temperature function, see Eq: \ref{eq:tempFunc}. }
	\label{fig:TempPost}
\end{figure}
\cleardoublepage
\begin{figure}[ht!]
