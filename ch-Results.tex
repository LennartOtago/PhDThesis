...
We use the MTC scheme to determine the marginal posterior distribution $\pi(\gamma, \lambda| \bm{y})$ over the hyperparameters $\delta$  and $\lambda = \delta / \gamma$ first and then the mean and covariance of the full posterior distribution $\pi(\bm{x}|\bm{y})$ as in Eq. \ref{eq:MeanInt} and \ref{eq:CovInt}.
We set $\bm{A} \coloneqq  \bm{A}_L$ and the mode $( \lambda_{0}, \gamma_0 )$ of the marginal posterior distribution, see Eq. \ref{eq:MargPostAppl}, is provided by the \texttt{scipy.optimize.fmin} function.
In doing so, we calculate the vector $\bm{B}^{-1}\bm{A}^T\bm{y} = (\bm{A}^T\bm{A} + \lambda \bm{L})\bm{A}^T\bm{y} $ and compute the determinant in $g(\lambda)$ using Cholesky decomposition.
Then we approximate $f(\lambda)$ with a 3rd order Taylor series and $g(\lambda)$ with a linear approximation in the log-space around the mode, where for the approximation in $g(\lambda)$ we set $\lambda_{max}$ to the maximum value of $\lambda$ on the TT-grid.
\begin{figure}[ht!]
	\centering
...
This takes about $0.1s$ and $400$ function evaluations.
Then we approximate the marginals $\pi(\lambda| \bm{y})$ and $\pi(\gamma| \bm{y})$  as in Eq. \ref{eq:MargTT}.
In doing so we calculate $\bm{B}$ and $\bm{B}_{\text{pre}}$ as in Prop. \ref{prob:backMarg} and \ref{prob:ForMarg}, where we set $\uplambda(x) = 1$, so that for Cartesian basis $\bm{M}_k = \text{diag}(\uplambda_k(\mathcal{X}_k))$ in Eq. \ref{eq:MassMat}.
...
Based on the marginal posterior distribution $\pi(\gamma, \delta | \bm{y})$ we calculate the mean and covariance of the conditional posterior $\pi(\bm{x} | \gamma, \delta, \bm{y})$ by quadrature as in Eq. \ref{eq:MeanInt} and Eq. \ref{eq:CovInt}.
We can either use the samples based histogram bins as weights or the TT approximation to integrate over the whole grid.
...
Similarly, we use the approximations of $\pi(\delta| \bm{y})$ and $\pi( \gamma | \bm{y})$ to calculate the weighted expectations of $\bm{\mu}_{\bm{x}|\bm{y}}$ and $\bm{\Sigma}_{ \bm{x}|\bm{y}}$.
...
In total that we have to evaluate $\bm{x}_{\lambda}$ and invert $\bm{B}_{\lambda}$ 25 times to obtain mean and covariance of $\pi(\bm{x}|\bm{y})$, Fig. \ref{fig:O3Samp}, which takes less than $0.2$s.
We plot posterior samples of $\pi(\bm{x}|\bm{y})$ in Fig. \ref{fig:O3Samp}, where we set negative values ozone VMR to zero.
Note that the sample mean is slight larger than posterior mean at heights where the data is noise dominated and the ozone values are determined by the prior.
This indicates that we should use a different more physical based, e.g. parametrised, prior at higher altitudes.
\begin{figure}[ht!]
	\centering
...
We focus on the posterior distribution of ozone profiles conditioned on pressure and temperature, as to characterise this posterior is a much quicker process, when using the MTC method, compared to obtaining the pressure and temperature posterior distributions.
\begin{figure}[htb!]
	\centering
	\begin{tikzpicture}
...
The subspace $\bm{W}$ is created  from the linear model and $\bm{V}$ by noise free data the non-linear model, given $m$ samples $\bm{x}^{(j)} \sim \pi(\bm{x}|\bm{y})$ for $j = 1, \dots,m$.
...
We asses the affine map by calculating the relative difference $\lVert \bm{M}\bm{W} - \bm{V}  \rVert_{L^2} / \lVert \bm{M}\bm{W} \rVert_{L^2} $ between the mapped linear noise free data and the non-linear noise free data, which is approximately $0.001\%$.
...
\begin{figure}[ht!]
	...
	\label{fig:MapAsses}
\end{figure}
In Fig. \ref{fig:MapAsses}, we show the mapping for one posterior ozone sample which has not been used to create this mapping, one could say an unseen event not in the training data.
The relative error for this mapping is roughly $0.03\%$ and much smaller than the relative difference between noise free linear data and non-linear data.
...
The marginal posterior is defined as in Eq. \ref{eq:MargPostAppl} but with $ \bm{A} \coloneqq \bm{M A}_L$.
...
Then we run the MWG algorithm for $N = 20000$ plus $N_{\text{burn-in}} = 100$ steps and plot the samples in Fig. \ref{fig:MargPostHistTT} as well as the marginal approximations provided by the TT decomposition, where we use the exact same setup as in Sec. \ref{subsec:firstMargTT}.
...
\subsection{Conditional Posterior Variance and Mean}
Next, we characterise the conditional posterior $\pi(\bm{x}|\bm{y})$ as in Eq. \ref{eq:CondPost}. 
Again, we calculate the full conditional mean $\bm{\mu}_{\bm{x}|\bm{y}}$ \ref{eq:MeanInt} and covariance matrix $\bm{\Sigma}_{ \bm{x}|\bm{y}}$ \ref{eq:CovInt} as weighted expectation over a 25-point grid provided by either the marginal TT approximations of $\pi(\gamma| \bm{y})$ and $\pi(\lambda| \bm{y})$ or by the bins of the sample histogram as quadrature weights.
We plot the conditional mean and variance in Fig. \ref{fig:O3SolplsReg} and the regularised solution and one sample from the posterior, which represents a feasible solution to the inverse problem.
We can see that the ground truth lays within 3 times the variance around the mean accounting for roughly $99 \%$ of all solution.
We also note that compared to the previously calculated mean and variance based on the linear forward model, see \ref{fig:O3Samp}, the posterior based on the approximated does not differ significantly.
\begin{figure}[ht!]
	\centering
...
We observe that the last 10 singular values are very small and correspond to the ozone values at the high altitudes $\gtrsim 45$ and have a large variance, see Fig. \ref{fig:O3SolplsReg}.
...
This does make sense, if one thinks about the mean (average) and the regularised solution as extremely smooth ozone profiles, see also Sec. \ref{sec:regularise}.
