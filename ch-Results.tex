...
We approximate and sample from the marginal posterior $ \pi(\gamma , \delta|\bm{y})$ and then charcterise the full conditional posterior distribution $\pi(\bm{x}|\bm{y})$ based on the linear forward model $\bm{A}_L$, neglecting absorption, see Eq. \ref{eq:RTE}.
...
We define a grid similar to the sampling region of the MWG sampler with 25 grid points in each dimension and use the \texttt{tt.cross.rectcross.rect\_cross.cross} function from the \texttt{ttpy} Python package, based on the rect cross algorithm in \cite{}, to calculate the cores of each dimension in less than $0.1$s.
We set the number of ranks to a constant value $r = 4$ and optimise over those ranks with one sweep.
...
With the height of the histogram bars as quadrature weights, e.g. $\pi(\lambda_i| \bm{y})$, where $\lambda_i$ is the centre of each bin we caculate the full conditional mean $\bm{\mu}_{\bm{x}|\bm{y}}$ and covariance matrix $\bm{\Sigma}_{\bm{x}|\bm{y}}$ as weighted expecations.
...
A feasible method is the Cholesky forward and backwards substitution, which takes roughly $1$s to compute the mean and variance.
...
The best regularised solution corresponding to the corner of the L-curve is located at the point of maximum curvature, see triangle in Fig. \ref{fig:LCurve}, which we find with the kneedle algorithm~\cite{satopaa2011kneedle} using the python function \texttt{kneed.KneeLocator} in roughly $2$ seconds.
\begin{figure}[ht!]
	\centering
...
It takes roughly $2$min for 15 sweeps by the \texttt{cross} to find the optimal tensors.
