...
\end{align}
as weighted expectations, by quadrature \cite[Sec. 2.1]{Dick_Kuo_Sloan_2013}
...
We approximate and sample from the marginal posterior$ \pi(\gamma , \delta|\bm{y})$ and then charcterise the conditional posterior distribution $\pi(\bm{x}|\bm{y})$ based on the linear forward model $\bm{A}_L$ which neglects absorption, see Eq. \ref{eq:RTE}.
Then, we can generate two affine subspaces $\bm{W} = \{\bm{A}_L\bm{x}^1, \dots, \bm{A}_L\bm{x}^m \}$ and $\bm{V} = \{\bm{A}_{NL}\bm{x}^1, \dots, \bm{A}_{NL}\bm{x}^m \}$, with $\bm{x}^{j} \sim \pi{\bm{x}|\bm{y}}$ for $j = 1, \dots,m$.
Finally we use the \texttt{numpy.linalg.solve} python function to solve $ \bm{M}\bm{W} = \bm{V}$, see Sec. \ref{sec:affine} for more details.
\begin{figure}[htb!]
	\centering
	\begin{tikzpicture}
...
Alternatively we can approximate marginal posterior with a tensor-train (TT) of the square root of marginal posterior on a predefined grid.
...
Finally, we are able approximate the non-linear forward model
\begin{align}
