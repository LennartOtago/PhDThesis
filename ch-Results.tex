...
Here we can already see that our model is not taking into account that ozone values can not be negative (see $\pi(\bm{x}|\delta)$ in Eq. \eqref{eq:priorXAppl}), and we currently have no means to include that in a non-parametric approach.

As already mentioned, we set the precision matrix of the prior distribution $\bm{x}|\delta$ to
\begin{align}
	\delta \bm{L} =
	\delta
	\begin{bmatrix}
...
Choosing gamma distributions for $\pi(\delta)$ and $\pi(\gamma)$ has another advantage when using a MWG sampler to sample from the marginal posterior distribution $\pi(\delta, \gamma | \bm{y})$.
...
\begin{subequations}
	\begin{align}
		\bm{y} |  \bm{x},\gamma,\delta  &\sim \mathcal{N}(\bm{A}(p_0,b,\bm{h}_{\bm{T}},T_0,\bm{a}) \, \bm{x}, \gamma^{-1} \bm{I}) \label{eq:likelihoodFull} \\
		\bm{x}| \delta  &\sim \mathcal{N}(\bm{0}, (\delta \bm{L})^{-1} ) \label{eq:priorXFull} \\
		\delta  &\sim \Gamma(\alpha_{\delta} = 1, \beta_{\delta} = 10^{-35})\label{eq:priorDelFull} \\
		\gamma  &\sim \Gamma(\alpha_{\gamma} =1, \beta_{\gamma} = 10^{-35})\label{eq:priorGamFull} \\
...
which we summarise in Tab. \ref{tab:priors}, and to formulate the posterior distributions.
