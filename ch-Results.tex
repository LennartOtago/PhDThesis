...
\begin{subequations}
	...
	\end{align}
\end{subequations}
for the linear model $A_L$, see Sec. \ref{subsec:MTC} for the derivation.
...
Here $\Delta f = f(\lambda^\prime) - f(\lambda^{(k)}) = \sum f^{(k)} (\lambda_0)\Delta \lambda^\prime - \Delta \lambda^{(k)} $ and $\Delta g = g(\lambda^\prime) - g(\lambda^{(k)})= \sum g^{(k)} (\lambda_0)\Delta \lambda^\prime - \Delta \lambda^{(k)} $, where $\Delta \lambda^\prime = \lambda^\prime - \lambda_0 $ and $\Delta \lambda^{(k)} =  \lambda^{(k)} - \lambda_0$.
...
Alternatively we can approximate the square root of marginal posterior with a Tensor-Train (TT) on a grid with $40$ grid points in each dimension.
...
We calculate the marginals $\pi(\lambda| \bm{y})$ and $\pi(\gamma| \bm{y})$ as in section \ref{}, with a constant $\gamma = $, and plot the TT approximation as a colour code on top of the obtained samples in the scatter plot in Fig. \ref{fig:ScatterPlotTT}.
...
Note that calculating the covariance can be expensive and if that is the case the RTO methods is the preferred choice.
...
With the samples $\{\bm{x}^{(1)} ,\dots, \bm{x}^{(m)}\}\sim \pi(\bm{x}|, \gamma, \lambda, \bm{y}) $ from the conditional posterior we generate two affine spaces
\begin{align}
	\bm{V} = \begin{bmatrix}
		\vert&   &  \vert & & \vert \\
...
As displayed in Fig. \ref{fig:MapAsses} we can approximate the non-linear forward model well \textcolor{red}{withitn the noise, which is??} and from here on will use the approximated forward map.
\begin{figure}[ht!]
	\centering
...
From now on updaeted forward map
\subsection{Hyper-parameters samples from the marginal posterior distribution}
\begin{figure}[ht!]
	\centering
...
\end{itemize}
On a MacBook Pro from 2019 with 2.4ghz quadcore intel core i5 it takes  TT takes $\approx 2$to max $4min$ $\epsilon = 1.5e-1$ or max number of sweeps 11
 where one sweep is left to rught to left
conditoned on an ozone smaple 
t-walk burnIn 1000 samples samples 1000000 takes $5-6$ mins
