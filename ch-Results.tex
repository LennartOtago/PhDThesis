...
We target Ozone ate \cite{waters2006earth}
...
\begin{align}
	\text{SNR} = \frac{max(y)}{rms noise} = \frac{max(y)}{std noise}
\end{align}
for noise with zero mean.
Note that we multiply the data with  to avoid numerical issues within the inversion process.
...
In \cite[8]{Froidevaux2008snrozone} a signal around 235.71 GHz fro targeting ozone is reported with a maximal signal strength of $ \approx 100$K with a noise range of 0.4 to 1.6 K is reported, which we related very roughly to a signal-to-noise ratio of around 100.
...
\section{Conditional Posterior Distribution of Ozone with approximated Model}
...
\subsection{Samples from Tensor-train approximation of the Marginal Posterior}
