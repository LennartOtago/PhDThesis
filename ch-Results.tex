...
\begin{figure}[ht!]
	...
	\caption[Singular values of the posterior covariance matrix]{Singular values of the precision matrix of $\bm{Q}_{ \bm{x}|\bm{y}} = (\sum_{\gamma,\lambda} \gamma^{-1}(\bm{A}^T \bm{A} + \lambda \bm{L})^{-1})^{-1}$ of the posterior distribution $\pi(\bm{x}|\bm{y})$ for ozone. Since $\bm{Q}_{ \bm{x}|\bm{y}} $ is symmetric and positive definite the singular values are equal the absolute value of the eigenvalues, $\sigma_i = |\uplambda_i|$.
