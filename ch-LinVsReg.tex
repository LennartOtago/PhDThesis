...
The univariate grid is defined over $\gamma = [ 0.8 \times 10^{15},1.2 \times 10^{16}]$ and $\lambda = [ 10^{-5}, 8 \times 10^{-4}]$ with $n = 20$ grid points (see Fig.~\ref{fig:MeanVarError} and Sec.~\ref{subsec:firstCond} where we argue for the number of grid points).
...
One sweep within the \texttt{ttpy.$(\cdots)$.cross} Python function results in $2n_{\text{sweeps}}2nr =400$ function evaluations and takes $\approx 0.02$s.
