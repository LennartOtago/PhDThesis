...
Note that usually a Taylor series includes a factor $(r!)^{-1}$, which in this case cancels in $f^{(r)}(\lambda_0)$, and $g(\lambda)$ can be approximated with a Taylor series as well (see~\cite{fox2016fast}).
We plot the approximations 
\begin{subequations}
	\label{eq:fandgapprox}
	\begin{align}
		&\tilde{f} ( \lambda) = \sum^2_{r=0} 	f^{(r)}(\lambda_0) (\lambda-\lambda_0)^r  \label{eq:fAprox} \, ,  \\
		&\text{and } \tilde{g}(\lambda)=  g(\lambda_{0}) + (\log{\lambda} - \log{\lambda_{0}})  \frac{ g(\lambda_{\text{max}}) - g(\lambda_{\text{min}}) }{\log{\lambda_{\text{max}}} - \log{\lambda_{\text{min}}} }  \label{eq:gAprox} \, ,
	\end{align}
...
Compared to a 1-st, 3-rd or 4-th order Taylor approximation, we find that the 1-st order Taylor approximation of $f(\lambda)$ gives the smallest relative RMS error of $\approx 15 \%$ in the sampling region of $\lambda$ and a absolute RMS error of $1.8\times 10^{-16}$.
Additionally, we find that a linear approximation of $g(\lambda)$ is sufficient with relative RMS $\approx 4\%$ and an absolute error of $0.74$.

These errors then propagate into the marginal posterior $\pi(\lambda , \gamma| \bm{y})$ so that the relative RMS error is $\approx 5 \%$ over the whole grid.
When sampling, we evaluate the acceptance ratio in the log-space, so we report a relative RMS error of $ \approx0.06\%$ for $\log{\pi(\lambda| \gamma, \bm{y})}$, with a maximum relative pointwise error of $\approx 0.3\%$.
...
\end{align}
where $\Delta \lambda = \lambda^{\prime} - \lambda^{(k)} $ and  $\Delta f \approx \tilde{f}(\lambda^\prime) - \tilde{f}(\lambda^{(k)}) =   (\Delta \lambda^\prime)- (\Delta \lambda^{(k)}) $, with  $\Delta \lambda^{\prime} = \lambda^\prime - \lambda_0 $ and $\Delta \lambda^{(k)} =  \lambda^{(k)} - \lambda_0$.
...
\textcolor{red}{We predefined the univariate grid with $n = 20$ grid points (see Fig. \ref{fig:MeanVarError}) over $\gamma = [ 0.8 \times 10^{15},1.2 \times 10^{16}]$ and $\lambda = [ 10^{-5}, 8 \times 10^{-4}]$.}
...
The relative RMS TT approximation error over the whole grid is $\approx 5\%$ and similar to the propagation error in $\pi(\lambda, \gamma| \bm{y})$ due to the approximations of $f(\lambda)$ and $g(\lambda)$ (see further up).
\begin{figure}[h!]
	\centering
