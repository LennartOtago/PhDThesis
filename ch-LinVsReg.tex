...
To compute the marginals $\pi(\lambda| \bm{y})$ and $\pi(\gamma| \bm{y})$ the constant in Eq.~\ref{eq:SQTTapprox} is set to $\xi = 1 / \uplambda (\mathcal{X})$ because we observe marginal posterior values of around $10^{47}$.
...
Here, we provide an error estimate for the posterior mean $\bm{\mu}_{\bm{x}|\bm{y}}$ and covariance $\bm{\Sigma}_{\bm{x}|\bm{y}}$ due to the grid size or number of bins of the marginal posterior. 
This is done by comparing to the mean and covariance obtained by weighted expectations over histograms with 200 bins for the samples from $\pi(\lambda,\gamma | \bm{y})$.
...
The relative RMS error behaves roughly proportional to $1/N$, and we consider a relative RMS error less than $0.5\%$ good enough, which is easily met at 20 bins.
