...
In this Chapter we set up an hierarchiacal Bayesian model and apply the MTC method to provide posterior ozone profiles.
In this chapter, we guide the reader through the process of setting up a Bayesian framework and ultimately obtaining the posterior distributions of hyper-parameters and parameters of interest, DAGs to visualise hierarchical and conditional depdenseies of the parameters establish a choice of prior distributions within our Bayesian model and formulate the posterior distributions.
We treat this inverse problem as a linear inverse problem by neglecting the absorption term in RTE (see Eq.~\ref{eq:RTE}).
We employ a Metrolpis within Gibbs sampler and a TT approxiamtion to characterise the marginal posterior and elaborate on some errors.
Then we calculate the mean and the covariance matrix ofposterior for ozone,
Lastly, we compare to a regualrisaiotn approach.
...
Compared to a 2nd, 3-rd or 4-th order Taylor approximation, we find that the 1-st order Taylor approximation of $f(\lambda)$ gives the smallest relative RMS error of $\approx 9 \%$ for $\lambda = [ 10^{-5}, 8 \times 10^{-4}]$ (TT grid) and a maximum absolute error of $\approx 3 \times 10^{-16}$.
Additionally, we find that a linear approximation of $g(\lambda)$ is sufficient with relative RMS $\approx 3\%$ and a maximum absolute error of $\approx 5$.
...
\label{subsec:MWG}
Using these approximations, we employ a Metropolis within Gibbs (MWG) sampler summarised in Alg. Box \ref{alg:MwG} to characterise $\pi(\lambda,\gamma|\bm{y})$ as in~\cite{fox2016fast}.
...
Then we approximate $f(\lambda)$ with a 1-st order Taylor series and $g(\lambda)$ with a linear approximation in the log-space, where for the approximation in $g(\lambda)$ we set $\lambda_{\text{max}}$ to the maximum value of $\lambda$ on the TT grid (see next section).
We initialised the MWG algorithm at the mode $(\lambda^{(0)} , \gamma^{(0)}  ) = ( \lambda_{0} , \gamma_{0}  )$ and take $N = 10100$ , which inlcudes burn in period of $N_{\text{burn-in}} = 100$ steps.
...
We predefined the univariate grid with $n = 20$ grid points (see Fig. \ref{fig:MeanVarError}) over $\gamma = [ 0.8 \times 10^{15},1.2 \times 10^{16}]$ and $\lambda = [ 10^{-5}, 8 \times 10^{-4}]$.
We introduce a ``normalisation constant'' $c = - 300/2$ to avoid underflow so that the values of $\sqrt{\pi( \lambda,\gamma| \bm{y})} = \exp \{ 0.5 \log  \pi(\lambda,\gamma | \bm{y}) + c \} $ are within computer precision.
...
Within this method we compute $\bm{x}_\lambda$, for 200 different $\lambda$ values in between $10^{0}$ to $10^{6}$ and plot the regularisation norm $\sqrt{\bm{x}_\lambda^T\mathbf{L} \bm{x}_\lambda}$ against the data misfit norm $\lVert \bm{A}\bm{x}_\lambda - \bm{x} \rVert_{L^2}$ (see Figure \ref{fig:LCurve}). 
