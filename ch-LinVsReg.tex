...
In this Chapter we guide the reader through the process of setting up a hierarchical Bayesian framework use a DAG to visualise conditional dependencies between hyper-parameters and parameters.
We establish a choice of prior distributions within our Bayesian model and applying the MTC we formulate the marginal and full conditional posterior distribution.
We treat this inverse problem as a linear inverse problem by neglecting the absorption term in RTE (see Eq.~\ref{eq:RTE}).
We employ a Metropolis within Gibbs sampler and a TT approximation to characterise the marginal posterior and elaborate on some errors.
Then we calculate the mean and the covariance matrix of the posterior distribution for ozone and compare to a regularisation approach.
