...
\mccorrect{This graph Laplacian introduces local dependence between neighbouring altitude layers and penalises large second-order differences in the profile. Consequently, the prior favours smooth ozone profiles while still allowing gradual changes with altitude. This reflects the physical expectation that atmospheric quantities vary smoothly over neighbouring atmospheric layers. Furthermore, the resulting precision matrix is sparse and positive definite, making it computationally attractive for Bayesian inference.}
...
Given the marginal posterior over the hyper-parameters, we can evaluate the normally distributed full conditional posterior distribution of ozone
\begin{align}
...
\end{align}
of $\pi(\bm{x}| \bm{y})$ are computed as weighted expectations over the marginal posterior $\pi(\lambda,\gamma | \bm{y})$ by quadrature \cite[Sec. 2.1]{Dick_Kuo_Sloan_2013} with $\sum \pi(\lambda_i| \bm{y}) = \sum \pi(\gamma_i| \bm{y}) = 1$.
