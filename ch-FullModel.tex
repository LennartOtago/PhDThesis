...
\end{align}
with $\bm{\theta}  \coloneqq \{p_0,b,T_0,\bm{h_T},\bm{a}  \}$ accounting for the all pressure and temperature related hyper-parameters and the affine approximation $\bm{M}$ from the previous Chapter.
...
\begin{subequations}
	...
	\end{align}
\end{subequations}
Due to Gaussian noise $\pi(\bm{y} |  \bm{x},\bm{\theta},\delta,\gamma )$ is a normally distributed likelihood and Eq.~\ref{eq:priorXFull} to Eq.~\ref{eq:lastprior} denote prior distributions.
...
\end{figure}
We start by describing the pressure $\bm{p}$ in between $h_{L,0} \approx 7$km and $h_{L,n} \approx 83$km with an exponential function
\begin{align}
	p(h) =
...
\begin{table}[ht!]
	...
	The hyper-parameters are ordered as in the TT format according to their correlation structure.}
	\label{tab:priors}
\end{table}
...
Hyper-parameters describing the temperature at higher altitudes are very much uncorrelated and the IACTs in Tab.~\ref{tab:priors} agree with those results.
