...
\begin{mccorrection} 
The hyper-prior distributions for $p_0$, $b$, $T_0$, $\bm{h}_T$, and $\bm{a}$ are defined as Gaussian distributions. To complete the model, sensible hyper-prior means and variances must be specified.
The variances of $\pi(\bm{h}_T)$ are chosen such that the temperature profile maintains its physical structure and satisfies $h_{T,i} < h_{T,i+1}$ for $i=1,\ldots,5$ (see Fig.~\ref{fig:HeightPriors}).
The means of $\pi(\bm{h}_T)$ and $\pi(\bm{a})$ are set to the reference values given in Tab.~\ref{tab:tempGrad} and reported in~\cite[Tab.~5]{atmosphere1976us}. The variances of $\pi(\bm{a})$ allow for a wide range of prior temperature profiles while remaining within a physically reasonable range.
The variance of $\pi(T_0)$ was chosen to reflect realistic atmospheric variability based on publicly available meteorological forecasts and data~\cite{meteoblue,ZoomEarth}. 
Specifically, the mean and variance of $\pi(T_0)$ are chosen to represent a daily temperature variability of approximately $30$K around the mean sea-level temperature of $288$K~\cite{atmosphere1976us}.
...
The variability of $\pi(p_0)$ is $\approx 80$hPa and close to what we can observe when looking at weather data~\cite{meteoblue,ZoomEarth}.
...
\mccorrect{These prior distributions are intended to represent plausible atmospheric variability rather than exact conditions and may be adjusted when the framework is applied to a specific geographical regions or seasons.}
...
\begin{mccorrection}Correlated variables are ideally placed close together in the TT ordering because this allows the correlation structure to be captured locally within a small number of TT ranks. As a result, only a few ranks need to become large, while the remaining ranks can stay comparatively small, improving computational efficiency.
If strongly correlated variables are placed far apart in the ordering, the correlation structure must propagate through multiple TT cores, causing many TT ranks to be large throughout the decomposition. This significantly increases the computational complexity of the approximation.
Alternatively, one could transform the parameter space to reduce correlations before constructing the TT approximation, e.g., through a whitening transformation or Cholesky-based decorrelation~\cite{KessyWhitening2018}. In such transformed coordinates, the dependence structure becomes weaker and lower TT ranks may be sufficient even for different orderings.
\end{mccorrection}
...
The ranks between TT cores are \mccorrect{fixed to be} constant.
...
Based on these results the grid size is set to $n_{\text{grid}} = 30$.
...
\mccorrect{The alignment between prior and posterior distributions can have different interpretations. In one case, the posterior may remain close to the prior because the data and forward model provide only limited information about the corresponding parameter. Alternatively, similar prior and posterior distributions may occur if the chosen prior distribution already closely resembles the posterior distribution. We tested this by varying the prior distribution and examining the resulting posterior, but could not observe a significant change in behaviour. In particular, the prior and posterior distributions remained closely aligned under different reasonable prior choices. This indicates that the available data is not sufficiently informative for these parameters. 
%If the posterior changes only slightly for various sensible priors, this further indicates that the inference is stable.
}
...
\begin{mccorrection} 
The RTO method is not strictly necessary to sample from the conditional posterior. However, it provides an efficient way to generate independent samples at the cost of a single linear solve together with the generation of $n+m$ standard Gaussian random variables. The RTO method avoids explicitly forming the posterior covariance matrix by exploiting the structure of the hierarchical Bayesian model, which becomes computationally expensive in high-dimensional settings.
\end{mccorrection} 
...
\begin{align}
	\bm{x}^{(k)} = \arg \min_{\bm{x}} \lVert \hat{\bm{A}} \bm{x} - ( \hat{\bm{y}} + \bm{b} ) \rVert_{L^2}^2 	\label{eq:minimizer}
\end{align}
...
\begin{mccorrection} 
Since the full conditional posterior is Gaussian, it is sufficient to show that the random variable generated by the RTO optimisation problem has the correct posterior mean and covariance. A detailed derivation proving that Eq.~\ref{eq:minimizer} generates an exact sample from the conditional posterior distribution is provided in Appendix~\ref{sec:RTO_additional}.
\end{mccorrection} 
...
Note that $\bm{v}_1, \tilde{\bm{v}}_1 \in \mathbb{R}^m$ and $\bm{v}_2, \tilde{\bm{v}}_2 \in \mathbb{R}^n$.
...
\begin{mccorrection}
	Note that the uncertainties in the posterior ozone distribution Fig.~\ref{fig:O3Post} are substantially larger than those obtained in the simpler hierarchical model with only two hyper-parameters (see Fig.~\ref{fig:O3Samp} and Fig.~\ref{fig:O3SolplsReg}). This is expected, since uncertainty in the temperature and pressure profiles is now incorporated within the broader model. Consequently, the posterior distribution reflects additional sources of uncertainty and dependencies between the atmospheric parameters and hyper-parameters.
\end{mccorrection}
