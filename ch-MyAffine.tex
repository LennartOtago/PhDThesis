...
An affine map $ \bm{z} = \bm{A}\bm{x} + \bm{b}$ maps a Gaussian $\bm{x} \sim \mathcal{N}(\bm{\mu}, \bm{\Sigma})$ onto a Gaussian $\bm{z} \sim \mathcal{N}(\bm{A}\bm{\mu} + \bm{b}, \bm{A}\bm{\Sigma}\bm{A}^T)$.
...
The resulting posterior mean and STD, and one sample of the full conditional $\pi(\bm{x}|\delta,\gamma,\bm{y})$, which represents a feasible solution to this inverse problem, are plotted in Fig.~\ref{fig:O3SolplsReg}.
...
\begin{figure}[ht!]
	\centering
	\includegraphics{My_SecRecResinclRegandSampl.png}
	%\includegraphics{SecRecResinclReg.png}
	\caption[]{Full posterior mean and variance and one ozone sample from the full conditional. These results are based on the approximated forward model $\bm{M}\bm{A}_L$.}
