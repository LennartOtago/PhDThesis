...
def SIRT(seeds, SQTT, univarGrid, BackMarg, absError):
	...
	# lebesgue measure for qudrature
	...
	# error as in "\cite[Eq. (35)]{cui2022deep}"
	gamError = absError / np.prod(WholeLebLam)
	
	# sample from first dimension "\cite[Eq. (35)]{cui2022deep}"
	firstMarg = gamError * lamX[0] + np.sum(BackMarg[0][0, :, :] ** 2, 1)
	# cumulative distribution function, normalised numerical
	...
	for n in range(0, numbSampl):
		# interpolate linear on grid in first dimension
		...
		for d in range(1, dim):
			...
			# piecewise polynomial interpolation, see Eq. "\ref{eq:LinPol}", as in "\cite{dolgov2020approximation}"
