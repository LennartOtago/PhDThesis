...
\begin{lstlisting}[language=Python, columns=flexible,  numbers=none, caption={Python code to calculate Backward marginals, as in Prop. \ref{prob:backMarg} and \cite{cui2022deep}.}, escapeinside={"}{"}]
...
\begin{minted}{python}
	def MargBack(TTCore, univarGrid):
	''' Backward marginalisation (see "Prop. \ref{prob:backMarg}")
	    as in SIRT from Cui and Dolgov"~\cite{cui2022deep}" '''
	...
	for k in range(dim - 1, 0, -1):
		r_kmin1, n, r_k = np.shape(TTCore[k])
		# "Eq.~\ref{eq:MassMat}, \cite[Eq.~22]{cui2022deep}" !! 
...
\end{minted}
\clearpage
\begin{lstlisting}[language=Python,  numbers=none, columns=flexible, caption={Python code to calculate forward marginals, as in Prop. \ref{prob:ForMarg}.}, escapeinside={"}{"}]
...
\clearpage
\begin{lstlisting}[language=Python,	 numbers=none, columns=flexible, caption = {Python code to draw samples via SIRT, as in Alg. Box \ref{alg:SIRT}.}, escapeinside={"}{"}]
def SIRT(seeds, SQTT, univarGrid, B, absError):
	...
	for n in range(0, numbSampl):
		...
		for d in range(1, dim):
			...
			CurrApprCore = np.copy(CurrApprCore) @ LinInterPol(SQTT[d], univarGrid[d], 
				sampls[d][n]) 
...
def getMargPDF(TTCore, univarGrid, absError):
	...
	# Lebesgue measure for quadrature Eq. 2.20
	WholeLebLam = np.zeros(dim)
	for k in range(0, dim):
		WholeLebLam[k] = (univarGrid[k][-1] - univarGrid[k][0])
	lamX = np.ones(dim)
	...
	for k in range(0, dim):
		if k == 0:
			...
			# last marginal PDF
			margPDF[k] = gamError * np.prod(WholeLebLam[:k]) + np.sum(Bpre[k][:, :, 0] ** 2, 	0) * WholeLebLam[k]
...
def LinInterPol(TTCore,univarGrid, sampl):
	'''  piecewise polynomial interpolation as in Dolgov et al.
