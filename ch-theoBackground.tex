...
\label{ch:background}
In this Chapter, we introduce and briefly derive the methods used throughout this thesis, and provide references for further reading. We keep it as general as possible, as the expressions specifically tailored towards the forward map will be presented in Chapter~\ref{ch:res}.
...
\begin{align}
	...
	\label{eq:NonLinDat}
\end{align}
based on a forward model $\bm{A}(\bm{x})$, which may be non-linear, an unknown parameter vector $\bm{x}$ and some additive random noise vector $\bm{\eta}$.
Naturally, due to the noise, which we classify through a hyper-parameter, we deal with a random process. \textcolor{red}{Maybe more clear is to say that the observation process (2.1) is a random process.}
...
We read $\pi(\bm{y}|\bm{x},\bm{\theta})$ as the distribution over $\bm{y}$ conditioned on the parameter $\bm{x}$ and the hyper-parameter $\bm{\theta}$.
If we write $\bm{\eta} \sim \pi_{\bm{\eta}}(\cdot|\bm{\theta})$, $\sim$ reads as ``is distributed as''.
Here $\bm{\theta}$ may account for multiple hyper-parameters, e.g. describing the distribution $\pi_{\bm{\eta}}(\cdot|\bm{\theta})$ over the noise vector $\bm{\eta}$, and describing physical properties or functional dependencies of $\bm{x}$, e.g. smoothness, through the prior distribution $\pi(\bm{x}|\bm{\theta})$.
Consequently we define a hyper-prior distribution $\pi(\bm{\theta})$, where $\pi(\bm{x}, \bm{\theta}) = \pi(\bm{x}|\bm{\theta}) \pi(\bm{\theta}) $.
Choosing these prior distributions is ultimately a modeller's choice and is crucial, as it shall not affect the posterior distribution \textcolor{red}{what, of course the prior affects the posterior. What are you trying to say?}
\begin{align}
	\pi(\bm{x},\bm{\theta}|\bm{y}) = \frac{ \pi(\bm{y} | \bm{x}, \bm{\theta} ) \pi(\bm{x}, \bm{\theta})}{\pi(\bm{y})} \propto \pi(\bm{y} | \bm{x}, \bm{\theta} ) \pi(\bm{x}, \bm{\theta}) \, ,
\end{align}
which, according to Bayes' theorem, gives us a distribution of $\bm{x}$ and $\bm{\theta}$ given (conditioned on) the data, with finite and non-zero $\pi(\bm{y})$. \textcolor{red}{two sentences, talk anbout posterior and then model }
Note that here we include the hyper-parameters within the posterior distribution, which is the key idea of hierarchical Bayesian modelling. \textcolor{red}{"the" key idea? Maybe say that it is intrinsic to Bayes models, or something. You could quote Kaipio and Somersalo that all unknowns are treated as random variables.}
We not only aim to quantify the posterior distribution over the parameters $\bm{x}$, but also the posterior distribution over the hyper-parameter $\bm{\theta}$.
We can visualise this hierarchically-ordered correlation structure between parameters as well as how distributions progress through a measurement process, using a directed acyclic graph (DAG) as in Figure~\ref{fig:FirstDAG}.
...
In practice, we approximate the estimation error from a sample set $\mathcal{M}^{(i)}$ as
\begin{align}
...
We define the integrated autocorrelation time (IACT) $\tau_{\text{int}}$ as in \cite{fox2016fast}, which is twice the value of the IACT in~\cite[pp. 103-105]{wolff2002LecNot}. \textcolor{red}{acknowledge, or 'take into account'}
...
\end{align}
into the full conditional posterior $\pi(\bm{x} |  \bm{\theta}, \bm{y})$ over the latent field $\bm{x}$ and the marginal posterior \textcolor{red}{, that can be calculated by,}
\begin{align}
...
\begin{align}
	...
	\label{eq:fullCond}
\end{align}
where, in the case of a linear-Gaussian hierarchical Bayesian model, both the marginal distribution $\pi (\bm{\theta}| \bm{y})$ \textcolor{red}{separate sentence (You have a tendency to let sentences wander to different ideas. Split into single topics.)} and the inner expectation $\mathbb{E}_{\bm{x} |  \bm{\theta}, \bm{y}} \left[ h(\bm{x}) \right]$ are well defined (see next subsection).
...
\label{subsec:LinBay}
In case of normally distributed noise $\bm{\eta} \sim \mathcal{N}(\bm{0},\bm{\Sigma}(\bm{\theta}))$ with zero mean and covariance $\bm{\Sigma}(\bm{\theta})$, a linear forward model matrix $\bm{A}$, and data \textcolor{red}{ Eq (2.1) simplifies to}
\begin{align}
	\bm{y} = \bm{A} \bm{x} + \bm{\eta} \, .
	\label{eq:LinDat}
\end{align}
\textcolor{red}{Then} we can derive the marginal and full conditional posterior distribution explicitly.
We define our hierarchical linear-Gaussian Bayesian model as
\begin{subequations}
	\label{eq:GenBayMode}
	\begin{align}
...
Immediately, we formulate the full conditional posterior distribution as 
\begin{align}
...
\begin{align}\begin{split}
	...
	\end{split} 
\end{align} 
Having the marginal posterior distribution $\pi (\bm{\theta}| \bm{y})$ (independent of $\bm{x}$) available breaks up the correlation structure between $\bm{x}$ and $\bm{\theta}$ (see Appendix~\ref{ap:Correlation}), and makes the MTC approach very efficient~\cite{fox2016fast}. \textcolor{red}{put this before the (), otherwise it reads very strangely}
...
\end{align}
of the full posterior \textcolor{red}{what is the "full posterior" as opposed to the "posterior"?} $\pi(\bm{x}| \bm{y})$ by some quadrature rule.
...
In the following, we describe how to compute marginal distributions from a probability density approximated in TT \textcolor{red}{ (tensor train)} format and how to generate samples using the inverse Rosenblatt transform (IRT), following the notation and procedure introduced by Cui et al.~\cite{cui2022deep}.

\textcolor{red}{I'm surprised that you never cite the Approximation an Sampling paper that introduced the idea of TT approximation of PDFs. Oh, you do, but the author list is abbreviated. Don't abbreviate the author list fro a thesis.}
...
\textcolor{red}{Here, we introduce a weight function $\uplambda(x)$, which can be useful for quadrature rules~\cite{davis2007methods}, which~\cite{cui2022deep} refers as a ``product-form Lebesgue-measurable weighting function'' and define it as}
\begin{align}
...
We call $\tilde{\pi}_k \in \mathbb{R}^{r_{k-1} \times n \times r_k}$ a TT core with ranks $ r_{k-1}$ and $r_k$, where the outer ranks are $r_0 = r_d = 1$, representing each dimension on $n$ grid points and connecting to neighbouring dimensions through its ranks. \textcolor{red}{more sentence creep. You should have defined the univariate grids, earlier.}
...
Instead of evaluating marginal functions for quadrature, we can draw samples from the approximated function in the TT format via the inverse Rosenblatt transform, as in \cite{dolgov2020approximation}, to preserve the correlation structure of the parameters. \textcolor{red}{what does this mean? If you draw from a distribution, it automatically has the right correlation structure. or are you trying to say something else, the I can't work out. }
Since we approximate the square root of the target function, Cui et. al.~\cite{cui2022deep} call that the squared inverse Rosenblatt transform (SIRT). \textcolor{red}{the 'we' seems quite inappropriate here, as it's Cui et al that call it SIRT.}
...
\begin{align}
	...
	\label{eq:CurrCDF}
\end{align}
and project the seed $u^{(s)}_k$ on the parameter space, so that we generate a sample $x^{(s)}_k = F_{X_k|X_{<k}}^{-1}(u^{(s)}_k)$. \textcolor{red}{Argghhhh!}
...
We repeat the procedure for each $u^{(s)}_k \in \bm{u}^{(s)}$ to produce samples $\bm{x}^{(s)} \sim f_{X}(\bm{x})$, as summarised in Alg.~Box~\ref{alg:SIRT}. \textcolor{red}{The procedure is repeated ... (the 'we' is entirely redundant, and distracting.)}
...
A straightforward way to assess the error from the TT approximation is to calculate the relative root mean squared (RMS) error \textcolor{red}{explain why this is a sensible thing to do.}
\begin{align}
...
\label{subsec:wasser}
Another way to assess the error between two distributions is to calculate the Wasserstein distance, which is the infimum over all couplings between two distributions weighted by some distance measure. \textcolor{red}{why is this sensible, or is it just 'another way'?}
...
So the 1-Wasserstein distance provides an upper error bound.
...
We could tackle the non-linearity by treating the inverse probkem as a linear inverse problem and then iteratively updating the non-linear part after each parameter sample.
