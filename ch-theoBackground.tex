...
\begin{figure}[ht!]
	...
	\caption[Hierarchical Bayesian Inference]{The directed acyclic graph (DAG) for an inverse problem visualises statistical dependencies as solid line arrows and deterministic dependencies as dotted arrows.
		...
		From the space of all measurable noise free data we observe a data set $\bm{y} = \bm{A}(\bm{x}) + \bm{\eta}$ with some random noise $ \bm{\eta} \sim \pi_{\bm{\eta}}(\cdot|\bm{\theta})$, which determines the likelihood function $\pi(\bm{y}|\bm{x},\bm{\theta})$. }
	\label{fig:FirstDAG}
\end{figure}
...
The expectation of any function $h(\bm{x})$, is described as 
\begin{align}
...
The central limit theorem states that the sample mean $ \bm{\mu}^{(i)}_{\text{samp}} $, of independent sample sets $\mathcal{M}_i$ for $i = 1, \dots, n$ of any distribution, converges to be normally distributed, so that
\begin{align}
...
We further asses this error in Sec. \ref{sec:sampling}.

\subsubsection{Marginal and then Conditional Method}
...
This approach, known as the marginal and then conditional (MTC) method, is particularly advantageous when $\bm{x}\in \mathbb{R}^n$ is high-dimensional, while $\bm{\theta}\in \mathbb{R}^{n_{\bm{\theta}}}$ is low-dimensional, so that $n_{\bm{\theta}} \ll n$ and evaluation of $\pi(\bm{\theta}| \bm{y})$ is cheap.
...
\begin{align}
	...
	\label{eq:fullCond}
\end{align}
where, in the case of a linear-Gaussian hierarchical Bayesian model, both the marginal distribution and the inner expectation $\mathbb{E}_{\bm{x} |  \bm{\theta}, \bm{y}} \left[ h(\bm{x}) \right]$ are well defined (see next subsection).
If the integral in Eq. \ref{eq:2fullCond} is expensive to calculate, we use sample based methods to produce a Markov chain $\{ (\bm{x}, \bm{\theta})^{(1)}, \dots, (\bm{x}, \bm{\theta})^{(N)} \} \sim \pi(\bm{x}, \bm{\theta} |  \bm{y}) $ and sample from $\pi(\bm{\theta} |  \bm{y})$ first and then draw samples the conditional $\pi(\bm{x} | \bm{\theta} , \bm{y})$. 




\paragraph{Linear-Gaussian hierarchical Bayesian model}
%\label{subsec:LinBay}
...
Within this scheme we evaluate the marginal posterior first and then either condition on hyper-parameters to draw conditional posterior samples $\bm{x} \sim \pi (\bm{x} | \bm{\theta}, \bm{y})$ (see Sec. \ref{subsec:RTO}) or evaluate mean
\begin{align}
...
\label{sec:sampling}
In this section we present the methods that draw samples from the marginal posterior $ \mathcal{M} = \{  \bm{\theta}^{(1)}, \dots,  \bm{\theta}^{(k)}, \dots, \bm{\theta}^{(N)} \} \sim \pi(\bm{\theta} |  \bm{y})$ as well as the randomise then optimise (RTO) method to draw samples from a normally distributed conditional posterior $\pi(\bm{x}|\bm{\theta} , \bm{y})$.
...
Then the chain converges to the desired target distribution  with $ \mathcal{M} \sim \pi( \bm{\theta} |  \bm{y})$.
...
If the Markov chain over the marginal posterior $\pi(\bm{\theta} |  \bm{y})$ is ergodic, and the conditional samples $\bm{x}^{(k)} \sim \pi(\bm{x}|   \bm{\theta}^{(k)}, \bm{y})$ are drawn independently, as described in Sec. \ref{subsec:RTO}, then the resulting joint chain $\{ (\bm{x}, \bm{\theta})^{(1)}, \dots, (\bm{x}, \bm{\theta})^{(N)} \} \sim \pi(\bm{x}, \bm{\theta} |  \bm{y}) =  \pi(\bm{x} |  \bm{\theta} , \bm{y}) \pi( \bm{\theta} | \bm{y})$ is also ergodic~\cite{acosta2022markov}.

\subsubsection{Integrated Autocorrelation time}
Assume we have a sample set $\mathcal{M}^{(i)} = \{\bm{x}^{(1)}, \dots,\bm{x}^{(k)},\dots, \bm{x}^{(s)},\dots, \bm{x}^{(N)}\} \sim \pi(\bm{x}|\bm{y})$ and we calculate the Monte-Carlo estimate
\begin{align}
\bm{\mu}^{(i)} \coloneqq	\text{E}_{\bm{x}|\bm{y}} [h(\bm{x})] = \frac{1}{N} \sum_{k=1}^{N} h(\bm{x}^{(k)}),
\end{align} 
then we have to asses the error $(\sigma^{(i)})^2 =\text{var}(\text{E}_{\bm{x}|\bm{y}} [h(\bm{x})])$ of that estimate.
This is because, any chain $\mathcal{M}^{(i)}$ produced by a system or an algorithm contains correlated samples and one has to take that into account when calculating sample-based estimates.
Here, we ignore systematic error due to initialisation bias (burn-in period), and derive the integrated autocorrelation time (IACT), following Ulli Wolff's lecture notes \cite{wolff2002LecNot}, with which we can express the error and which gives us a notion on how efficient our sampler is.

In general, the error of a Monte-Carlo-based estimate can be written as
\begin{align}
...
\begin{align}
	\Bigg( \frac{1}{N} \sum_{k=1}^{N} h(\bm{x}^{(k)}) -\bm{\mu}^{(i)} \Bigg)^2  =  \frac{\text{var}(h(\bm{x})) }{N^2} \sum_{k,s=1}^{N}\rho(k-s)\, ,
\end{align}
with the normalised auto correlation coefficient $\rho(k-s) =  \Gamma(k-s)/ \Gamma(0)$ at lag $k-s$, where the auto correlation coefficient $\Gamma(k-s) =  \big( h(\bm{x}^{(k)}) - \bm{\mu}^{(i)} \big) \big(h(\bm{x}^{(s)}) - \bm{\mu}^{(i)} \big)$ and  $\Gamma(0) = \text{var}(h(\bm{x}))$ for $k=s$.
...
\end{align}
where we define the IACT as in \cite[pp. 103-105]{wolff2002LecNot}.
We point out that if $2\tau_{\text{int}} = 1$ (uncorrelated samples) the error $(\sigma^{(i)})^2$ behaves similarly to a typical Monte-Carlo estimate.
...
According to that we define the effective sample size as $\frac{ 2 \tau_{\text{int}} }{N}$.
Since this is an estimate \cite{wolff2004monte} provides a way to not only an algorithm to calculate the IACT but also quantifies the errors of the estimate of the IACT.
...
If the number of hyper-parameters is large, we sample from the marginal posterior $\pi(\bm{\theta} | \bm{y})$ using the \texttt{t-walk} sampler as by Christen and Fox~\cite{christen2010general}. 
The \texttt{t-walk} is employed as a black-box algorithm, requiring the specification of the number of samples, burn-in period, support region, and the target distribution. 
Convergence to the target distribution is guaranteed by construction of this algorithm.


\subsection{Randomise then Optimise}
\label{subsec:RTO}
If it is computationally not feasible to calculate the mean and the covariance matrix of the full posterior (see Eq. \ref{eq:MeanGenInt} and \ref{eq:CoVarGenInt}) via quadrature due to a larger number of hyper-parameters, e.g. $\bm{\theta} \in \mathbb{R}^{n_{\bm{\theta}}}$ and $n_{\bm{\theta}} \geq 4$, we need an alternative way to characterise $\pi(\bm{x} |  \bm{\theta}, \bm{y})$.
For the linear-Gaussian Bayesian model we can draw samples from the normal distribution $\pi(\bm{x} |  \bm{\theta}, \bm{y})$ using the RTO~\cite{bardsley2012mcmc} scheme after sampling from the marginal posterior $\pi(\bm{\theta} |  \bm{y})$.

The full conditional distribution can be rewritten as
\begin{align}
	\pi(\bm{x} | \bm{\theta},  \bm{y}) = \exp \left( -\left\lVert \hat{\bm{A}} \bm{x} - \hat{\bm{y}} \right\rVert_{L^2}^2 \right),
\end{align}
where we define
\begin{align}
	\label{eq:minimizer}
...
\end{align}
Here we define $\bm{A}(\bm{\theta}) \coloneqq \bm{A}_{\bm{\theta}}$, $\bm{Q}(\bm{\theta}) \coloneqq \bm{Q}_{\bm{\theta}} $ and $\bm{\Sigma}^{-1}(\bm{\theta})\coloneqq \bm{\Sigma}^{-1}_{\bm{\theta}}$, which are all dependent on the hyper-parameters $\bm{\theta}$.
...
\begin{algorithm}[!ht]
	...
	\label{alg:SIRT}
\end{algorithm}
Since the samples using the SIRT scheme are samples from an approximation it is sensible to correct those using a Metropolis-Hastings (MH) importance sampler.
