...
Since we draw samples after each pther they are correalted adn we have to asses this coorelation in order to determine this effec on th e error or how efficnet sdampes chains are.
The error of an Monte-Carlo estimate based on one Markov-Chain $\mathcal{M}_i$ is:
\begin{align}
...
\end{align}
We can expand this summation so that
\begin{align}
...
\begin{align}
	\frac{1}{N} \sum_{k,s=1}^{N}	C(k-s) \approx \sum_{t = - \infty }^{\infty} \rho(t)
\end{align}
...
\begin{align}
	(\sigma^{(i)})^2 = \frac{\text{var}(h(\bm{x}_{\bm{\theta}}) )}{N^2} \sum_{k,s=1}^{N} \rho(k-s)  \approx  \frac{\text{var}(h(\bm{x}_{\bm{\theta}}) )}{N} \underbrace{\sum_{t = - \infty }^{\infty} \rho(t)}_{	2\tau_{\text{int}} } = \text{var}(h(\bm{x}_{\bm{\theta}})) \frac{ 2 \tau_{\text{int}} }{N} \, ,
\end{align}
