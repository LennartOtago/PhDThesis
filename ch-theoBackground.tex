...
In practice, one can inspect the trace $\pi(\bm{\theta}^{(k)} |  \bm{y})$ for $k = N_{\text{burn-in}}, \dots, N$ after a ``burn-in'' period $N_{\text{burn-in}}$ (to remove initialisation bias) and visually assess if the chain is consistent with ergodicity.
