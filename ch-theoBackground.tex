...
Specific sampling algorithms are not introduced here, as they are specifically tailored towards the structure of the forward model and the particular problem. Hence, they will be presented in detail when applied.
...
If $\bm{x}$ cannot be parametrised directly in terms of the hyper-parameters $\bm{\theta}$, and only a low-level representation of $\bm{x}$ is available, it is beneficial to factorise the joint posterior distribution
\begin{align}
...
If a high-level model of $\bm{x}$ is available, we can represent the parameter space of $\mathbb{R}^n$ with a number of hyper-parameters much smaller than $n$.
...
This drastically reduces the dimensionality of the joint posterior and that is when Bayesian modelling is especially powerful because we can impose prior knowledge through those hyper-parameters much more precisely compared to a low-level model.
...
In practice, one can inspect the trace $\pi(\bm{\theta}^{(k)} |  \bm{y})$ for $k = N_{\text{burn-in}}, \dots, N$ after a ``burn-in'' period $N_{\text{burn-in}}$ which removes initialisation bias and visually assess if the chain is consistent with ergodicity.
...
If the instance $\bm{\theta}^{(k)}$ of an ergodic Markov chain represents an independent sample of the marginal posterior $\pi(\bm{\theta} | \bm{y})$ and if a full conditional posterior sample $\bm{x}^{(k)} \sim \pi(\bm{x}|   \bm{\theta}^{(k)}, \bm{y})$ is drawn independently, as in Sec.~\ref{subsec:RTO}, then the resulting sample $(\bm{x}^{(k)}, \bm{\theta}^{(k)})$ is an independent sample from the joint posterior $\pi(\bm{x},   \bm{\theta} | \bm{y})$~\cite{fox2016fast,acosta2022markov}.
...
Instead of relying on sampling-based methods to explore a target function $\pi(\bm{x})$, which in our case will be the marginal posterior distribution over the hyper-parameters, we can approximate this function using a tensor-train (TT) approximation.
...
In the following, we describe how to compute marginal functions $f_{X_k}(x_k)$, for an $x_k \in \bm{x}$ and $k = 1, \dots,d$, from a target function $\pi(\bm{x})$ approximated in TT format.
Further, a scheme similar to the inverse Rosenblatt transform (IRT) in~\cite{dolgov2020approximation} is introduced to generate samples of $\pi(\bm{x})$.
...
The marginal function for the $k$-th component is then given by
\begin{align}
...
To approximate a target function in the TT format, one has to predefine $d$-dimensional discrete univariate grid over the parameter space $\mathcal{X}$ with $n$ grid points in each direction.
In the TT format, the integral in Eq.~\ref{eq:margInt} for the marginal function can be computed at a low computational cost, as $\pi(\bm{x})$ is approximated by
\begin{align*}
...
Consequently, the marginal function
\begin{align}
	\begin{split}
...
In particular when the target function is non-negative (e.g.~a probability distribution) the TT approximation can have negative values in regions where true function values are very small.
%because it is not advantageous yet to approximate the target function $\pi(\bm{x})$ in e.g. the logarithmic space. 
One way to ensure positivity is to square the target function, hence~\cite{cui2022deep} approximate the square root of the target function and 
...
The approximated unnormalised (probability density) function is written as \cite[Eq.~19]{cui2022deep}:
\begin{align}
	\pi(\bm{x}) \approx \xi + \tilde{g}(\bm{x})^2\,  ,
\end{align}
where $\xi$ is a positive constant added according to the ratio of the Lebesgue weighted L2-norm error and the Lebesgue weighting (see Eq.~\ref{eq:lebesgueWeight} and \cite[Eq.~35]{cui2022deep}) such that 
\begin{align}
	0 \leq \xi \leq \frac{1}{\uplambda(\mathcal{X})} \lVert \tilde{g} - \sqrt{\pi} \rVert_{L^2_{\uplambda}(\mathcal{X})}^2\,  \label{eq:gamErr}.
\end{align}
This leads to the normalised (probability density) function \cite[Eq.~19]{cui2022deep}
\begin{align}
...
\label{subsec:TTMarg}
The marginal functions $f_{X_k}(x_k)$ of the target function $f_{X}(\bm{x})$ are computed by a procedure to which Cui and Dolgov~\cite{cui2022deep} refer to as backwards marginalisation, see Prop.~\ref{prob:ForMarg}, and to which we add the forward marginalisation, see Prop.~\ref{prob:backMarg}. 
...
The backwards marginalisation provides the coefficient matrices $\bm{B}_k$, while the forward marginalisation gives the coefficient matrices $\bm{R}_{\text{pre}, k}$. 
These matrices enable the efficient evaluation of marginal functions since they are formed by integration over the coordinates either left or right of the $k$-th dimension, as in~\cite{cui2022deep}. 
...
After computing the coefficient tensors $\bm{R}_{\text{pre},k-1}$ as in Prop.~\ref{prob:ForMarg} and $\bm{B}_{k}$ from Prop.~\ref{prob:backMarg}, the marginal function of $k$-th dimension can be expressed as
\begin{equation}
...
Note that in practice we calculate $z$ numerically within the process of computing the marginals so that $\sum f_{X_k}(x_k) =1 $ and for Cartesian basis $\bm{M}_k = \text{diag}(\uplambda_k(\mathcal{X}_k))$ with $\uplambda(x) = 1$.
...
The idea is that a target function can be represented as the sequence $ f_X(\bm{x})  =   f_{X_1}(x_1)  f_{X_2|X_1}(x_2|x_1) \cdots  f_{X_k|X_{<k}}(x_k|x_{k-1},\dots,x_1)$ and within the IRT scheme samples are iteratively drawn from each $f_{X_k|X_{<k}}(x_k|x_{k-1},\dots,x_1)$ conditioned on the previous $k-1$ samples, for $k = 2 , \dots, d$.
...
\begin{algorithm}[!th]
	...
	\label{alg:SIRT}
\end{algorithm}
Given the backward marginal coefficient tensors $\bm{B}_1 , \dots,\bm{B}_d$ as in Prop.~\ref{prob:backMarg}, draw $N$ uniformly distributed seeds $\{ \bm{u}^{(1)},\dots, \bm{u}^{(N)} \} \sim \mathcal{U}(0,1)^d $, where each $\bm{u}^{(s)}$ is $d$-dimensional for $s = 1, \dots, N$.
Then calculate the first marginal $f_{X_1}(x_1)$ as in Eq.~\ref{eq:firstMarg} and normalise with $z = \int_{\mathcal{X}_1} f_{X_1}(x_1) d x_1$. 
...
Next, compute the cumulative distribution function (CDF) $F_{X_1}(x_k) = \int^{x_k}_{-\infty} f_{X_1}(\hat{x}_1) d \hat{x}_1$, which for the general case is given as \cite[Eq. 17]{cui2022deep}:
\begin{align}
...
The ``conditional marginal'' is given as \cite[Eq.~31]{cui2022deep}:
\begin{align}\begin{split} 
...
\label{subsec:wasser}
If large errors occur in regions with low probability, the RMS is sensitive to those, whereas the Wasserstein distance weights differences according to their respective probability values.
...
For example, if the regulariser imposes smoothness, the solutions will be very smooth and not sensitive to structures from the data.
...
\begin{figure}[ht!]
	...
	\caption[]{This Figure is directly inspired by~\cite{ColinPhD} and shows how a solution $ \bm{x}_{\lambda}  = \underset{\bm{x}}{\text{arg min }}\bm{x}^T \bm{L} \bm{x}$ subject to the constraint $\left\lVert \bm{y} - \bm{A} \bm{x} \right\rVert^2_{L^2}= c$ is obtained.}
	\label{fig:Langrangian}
\end{figure}
Alternatively one can introduce a Lagrangian $\mathcal{L}(\bm{x},\lambda)\coloneqq \left\lVert\bm{y}-\bm{A}\bm{x}\right\rVert^2_{L^2} + \lambda\bm{x}^T\bm{L} \bm{x}$ similar to \cite{LiLagrange}, where $\lambda$ is a Lagrange multiplier.
A solution $\bm{x}_{\lambda} $ that minimises $\mathcal{L}(\bm{x},\lambda)$ is usually obtained by finding the minimum of $\left\lVert \bm{y} - \bm{A} \bm{x} \right\rVert^2_{L^2}$ with respect to the constant constrained $\bm{x}^T \bm{L} \bm{x} = c$.
This is equivalent to $\bm{x}_{\lambda}  = \underset{\bm{x}}{\text{arg min }}\bm{x}^T \bm{L} \bm{x}$  subject to constant $\left\lVert \bm{y} - \bm{A} \bm{x} \right\rVert^2_{L^2}= c$ (see~\cite[fn. 6]{fox2016fast} and~\cite[Fig. 2.13]{SANTOSH202265}). 
...
So every solution $\bm{x}_{\lambda}$ is extremely regularised for a given data misfit and \textcolor{red}{the L-Curve presents the limit to on an open set of $\bm{x}$}.
Hence, every sample of the posterior, which represents a feasible solution given the data, lies above the L-Curve because it is less regularised and has a higher $ \bm{x}^T \bm{L} \bm{x}$ value.
\textcolor{red}{So regularised solutions do not represent the posterior parameter space.}
