...
\begin{align}
	W(\pi,\tilde{\pi})  =&\underset{ |h(\bm{x}) - h(\bm{y})| \leq  d(\bm{x},\bm{y}) }{ \text{sup}} \Bigg\{  \int_{\mathcal{X}} h(\bm{x}) d \pi (\bm{x})  - \int_{\mathcal{X}} h(\bm{y}) d \tilde{\pi} (\bm{y}) \Bigg\} \\
