...
\begin{align}
	W_2(\pi,\tilde{\pi}) = \lVert \bm{\mu} -\tilde{\bm{\mu}} \rVert^2_{L^2} + \text{trace}(\bm{\Sigma}+  \tilde{ \bm{\Sigma}} - 2 (\tilde{ \bm{\Sigma}}^{1/2} \bm{\Sigma} \tilde{ \bm{\Sigma}}^{1/2} )^{1/2} ) \, \text{\cite{Asuka2011Wasser}.}
\end{align}
We can write the Wassertein distance as the infimum of expectations
\begin{align}
	W_2(\pi,\tilde{\pi}) =  \underset{  \nu \in \Pi(\pi,\tilde{\pi}) }{ \text{inf}} \Bigg( \underset{ \bm{x},\bm{y} \sim  \nu  }{\mathbb{E}}  d(\bm{x},\bm{y})^2 \Bigg)^{1/2}
\end{align}
...
Then we can bound the absolute difference in expexations for a 1-Lipschitz function $h$ by the 2-Wasserstein distance $W_2(\pi,\tilde{\pi})$.
