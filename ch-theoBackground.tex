...
Naturally, due to the noise, we deal with a random process and we have to include that in our modelling.
Then define the likelihood function $\pi(\bm{y}|\bm{\theta},\bm{x})$ according to the nature of the noise as well as include all relevant information about the measurement process.
...
Choosing these prior distributions is crucial, as it shall not affect the posterior distribution 
\begin{align}
...
\begin{align}
	(\sigma^{(i)})^2   \approx  \frac{\text{var}(h(\bm{x}_{\bm{\theta}}) )}{N} \underbrace{\sum_{t = - \infty }^{\infty} \rho(t)}_{ \coloneqq 	2\tau_{\text{int}} } = \text{var}(h(\bm{x}_{\bm{\theta}})) \frac{ 2 \tau_{\text{int}} }{N} \, , \label{eq:MCerr}
\end{align}
...
We calculate the IACT using the Python implementation of \cite{WOLFF2004143}, accessed \cite{wolff2004monte}, which also provides errors on the IACT and helps to assess convergence of the chain.
\begin{align}
	\rho(t) \overset{t \rightarrow \infty }{=} \exp \Big\{ - \frac{t}{\tau} \Big\} 
\end{align}
then for $\tau \gg 1$ then $\tau_{int} \approx \tau$
\begin{align}
...
power series  geometric series formula 
...
Once we have draw samples from the TT approximation we have a few options to quantify the approximation error.
...
Another way is to do a Metropolis-Hastings correction step and to calculate the Wasserstein distance \cite{} between approximation and the true function.
In higher dimensions this might not be possible so we can compare the samples from a trusted sampler, which samples in the log space to the samples from the SIRT after a Metropolis-Hastings correction step as in \cite{dolgov2020approximation}.
This gives us marginal distributions to compare by calculating the Wasserstein distance.
...
\end{align}
as described in~\cite{fox2016fast}, with a linear forward model matrix $\bm{A}$, the data $\bm{y}$, a regularisation operator $\bm{T}$ and the regularisation parameter $\lambda > 0$ which weights the semi norm and penalises $\bm{x}$ according to that.
...
In practice, $\bm{x}_{\lambda}$ is computed for a range of $\lambda$-values and evaluated based on the trade-off between the data misfit and the regularisation semi-norm. The optimal value of $\lambda$ corresponds to the point of maximum curvature of the so-called L-curve~\cite{hansen1993use}, where we plot data misfit norm versus the regularisation semi-norm, see Fig.~\ref{fig:LCurve}.
...
So every solution $\bm{x}_{\lambda}$ is an extremum and almost every sample of the posterior, which represents a feasible solution given the data, has a higher $\bm{x}^T \bm{L} \bm{x}$ value and lays above the L-Curve.
