...
A observations process naturally includes some random noise which we can include in some hyper-parameters $\bm{\theta}$ and order a Bayesian model hierarchically.
 added to some obersovation from the space of all measurables $\bm{u}$.
...
\begin{subequations}
	...
	\label{eq:BayMode}
\end{subequations}
with the noise covarince matrix $\bm{\Sigma}(\bm{\theta})$, so that $\bm{\eta}  \sim \mathcal{N}(0, \bm{\Sigma}(\bm{\theta})) $ as in Eq. \ref{}, the prior precision matrix $\bm{Q}(\bm{\theta})$, prior mean $\bm{\mu}$
and some distribution over the hyper-parameters $\pi(\bm{\theta})$.
Then this becomes a linear-Gaussian Bayesian hierarchical model.

Describe Prior likelihood 
...
If the normalising constant $\pi(\bm{y})$ is finite and non-zero we can approximate the posterior distribution
\begin{align}
