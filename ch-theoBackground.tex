...
\begin{align}
\underset{\text{all comb. of $i$ and $j$}}{\text{min}} \sum^M_{j = 1} \sum^N_{i =1} \pi(\bm{x}^{(i)}) \tilde{\pi}(\tilde{\bm{x}}^{(j)}) \lVert\bm{x}^{(i)}  -  \tilde{\bm{x}}^{(j)} \rVert_{L^2}
\end{align}
\cite[Eq. 3.166]{feydy2020OT}
where $\sum^N_{i =1} \pi(\bm{x}^{(i)}) = \sum^M_{j = 1} \tilde{\pi}(\tilde{\bm{x}}^{(j)})= 1 $
