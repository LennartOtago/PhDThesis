...
\mccorrect{
	This chapter is intentionally formulated in a general setting. The prior distributions, parameter domains, and other problem-specific modelling assumptions are introduced later when the corresponding applications are considered. Here, we focus on the general hierarchical Bayesian framework and establish the notation and methodology for the class of hierarchical linear-Gaussian inverse problems considered in this thesis.
}
...
\begin{figure}[ht!]
	\centering
	\begin{tikzpicture}
		\node[roundnode2] at (-1.5,3.5) (thn)    {\mccorrect{$\bm{\theta}_{\bm{\eta}}$}};
				\node[roundnode2] at (0,3.5) (thx)    {\mccorrect{$\bm{\theta}_{\bm{x}}$}};
		\node[roundnode2] at (0,1.5) (x)    {$\bm{x}$};
		\node[roundnode2] at (0,-1.5) (u)    {$\Omega$};
		\node[rectnode] at (0,-3.5) (y)    {$\bm{y}$};
		
		\draw[->, very thick] (thx.south) -- (x.north); 
		\draw[->, very thick, mydotted] (x.south) -- (u.north); 
		\draw[->, very thick] (u.south) -- (y.north); 
		\draw[->, very thick] (thn) edge[bend right=20] (y);  
		
		\node[align=center] at (4,3.5) (tht) {\mccorrect{$\{\bm{\theta}_{\bm{\eta}},\bm{\theta}_{\bm{x}}\} = \bm{\theta}\sim \pi_{\bm{\theta}}(\cdot) $ }hyper-parameters};
		\node[align=center] at (2.4,1.5) (xt) {$\sim \pi_{\bm{x}}(\cdot|\bm{\theta}_{\bm{x}}) $ parameters};
		...
		\node[align=center] at (-3,0) (nt) {noise\\$\bm{\eta} \sim \pi_{\bm{\eta}}(\cdot|\bm{\theta}_{\bm{\eta}})$};
		
		\node[fit=(thn)(thx),draw,thick,dotted,black, rounded corners] {};
	\end{tikzpicture}
	\caption[Hierarchical Bayesian Model]{A directed acyclic graph (DAG) for an inverse problem visualises statistical dependencies as solid line arrows and deterministic dependencies as dotted arrows.
		The hyper-parameters $\bm{\theta} = \{\bm{\theta}_{\bm{\eta}},\bm{\theta}_{\bm{x}}\} $ are distributed as ($\sim$) the hyper-prior distribution $\pi(\bm{\theta})$.
		The prior distribution $ \pi_{\bm{x}}(\cdot|\bm{\theta}_{\bm{x}})$ for the parameter $\bm{x}$ and the noise  $\bm{\eta} \sim \pi_{\bm{\eta}}(\cdot|\bm{\theta}_{\bm{\eta}})$ are statistically dependent on some of those hyper-parameters.
		Then a parameter $\bm{x} \sim \pi_{\bm{x}}(\cdot|\bm{\theta}_{\bm{x}})$ is deterministically mapped onto the space of all measurables $\Omega$ through the forward model $\bm{A}(\bm{x})$.
...
Here $\bm{\theta}=\{\bm{\theta}_{\bm{\eta}},\bm{\theta}_{\bm{x}}\}$ may describe multiple hyper-parameters.
The hyper-parameters can e.g.,~model the noise vector $\bm{\eta} \sim \pi_{\bm{\eta}}(\cdot|\bm{\theta}_{\bm{\eta}})$, where $\sim$ reads as ``is distributed as'', and $\bm{\theta}_{\bm{x}}$ may account for physical properties or functional dependencies of $\bm{x}$, such as the smoothness of $\bm{x}$.
Because unknown parameter are treated as random variables the joint prior distribution is introduced as $\pi(\bm{x}, \bm{\theta}) = \pi(\bm{x}|\bm{\theta}_{\bm{x}}) \pi(\bm{\theta})$ with the parameter prior distribution $\pi(\bm{x}|\bm{\theta}_{\bm{x}})$ and the hyper-prior distribution $\pi(\bm{\theta})$.
...
\begin{align}
	\sqrt{n} (\bar{h}^{(i)}_N -  \bar{h} ) \overset{\mathcal{D}}{\longrightarrow} \mathcal{N} (0,\sigma^2)
\end{align}
and if $\sigma^2 < \infty$ the Monte-Carlo error $\bar{h}^{(i)}_N -  \bar{h} $ is bounded \cite{geyer1992practical}.
...
\begin{mccorrection}
The autocorrelation coefficient $\Gamma(t)$ itself is approximated by
\begin{align*}
	\Gamma(t)
	\approx
	\frac{1}{N-t}
	\sum_{i=1}^{N-t}
	\left(h(\bm{x}^{(i)}) - \bar{h}_N\right)
	\left(h(\bm{x}^{(i+t)}) - \bar{h}_N\right),
\end{align*}
where $\bar{h}_N$, as defined in Eq.~\ref{eq:sampMean}, denotes the sample mean~\cite[p. 106]{wolff2002LecNot}.
This estimator is important in practice because the true autocorrelation function is generally unknown and only a finite correlated sample set is available.
The estimated autocorrelation coefficients are then used to approximate the integrated autocorrelation time $\tau_{\text{int}}$.
\end{mccorrection} 
Choosing the summation window $W$ in Eq.~\ref{eq:MCerr} is crucial because it has to be large compared to the decay time $\tau$, but for too large $t$ the autocorrelation coefficient $\Gamma(t)$ is noise-dominated.
...
According to the IACT, the effective sample size is defined as $N/ \tau_{\text{int}}$.
...
\end{align}
into the full conditional $\pi(\bm{x} |  \bm{\theta}, \bm{y})$ for the latent field $\bm{x}$ and the marginal posterior $ \pi(\bm{\theta} |   \bm{y})$ over hyper-parameter $\bm{\theta}$.
...
\end{align}
as in~\cite[Lemma 2]{fox2016fast} \mccorrect{can be carried out efficiently.}
\begin{mccorrection}
The factorisation underlying the MTC framework is not restricted to the later introduced linear-Gaussian setting. Eq.~\ref{eq:margGen} holds more generally for hierarchical Bayesian models, see \cite[Lemma 2]{fox2016fast} and \cite{norton2018sampling}.
Furthermore, as discussed in \cite{norton2018sampling}, the MTC framework is particularly useful when the full conditional distribution
\begin{align*}
	\pi(\bm{x}|\bm{\theta},\bm{y})
	=
	\frac{\pi(\bm{y}|\bm{x},\bm{\theta})\pi(\bm{x}|\bm{\theta})}
	{\pi(\bm{y}|\bm{\theta})}
\end{align*}
has a known form. In this case, the marginal likelihood
\begin{align*}
	\pi(\bm{y}|\bm{\theta})
	=
	\int
	\pi(\bm{y}|\bm{x},\bm{\theta})
	\pi(\bm{x}|\bm{\theta})
	,\mathrm{d}\bm{x}
\end{align*}
can be evaluated and hence the marginal posterior $	\pi(\bm{\theta} |   \bm{y}) \propto 	\pi(\bm{y}|\bm{\theta}) 	\pi(\bm{\theta}) $.
However, the computational efficiency of the MTC approach depends strongly on the model structure. In the linear-Gaussian setting considered in this thesis, the full conditional for $\bm{x}$ and marginal likelihood of the hyper-parameters can be evaluated efficiently, making the MTC scheme particularly attractive. 
\end{mccorrection}
...
If the integral in Eq.~\ref{eq:2fullCond} is expensive to calculate, we use sample-based methods to produce a Markov chain $\{ (\bm{x}, \bm{\theta})^{(1)}, \dots, (\bm{x}, \bm{\theta})^{(N)} \} \sim \pi(\bm{x}, \bm{\theta} |  \bm{y}) $ and sample from $\pi(\bm{\theta} |  \bm{y})$ first and then draw samples from the full conditional $\pi(\bm{x} | \bm{\theta} , \bm{y})$ (see Sec.~\ref{subsec:RTO}). 
\mccorrect{By integrating out the latent field $\bm{x}$, the MTC approach shifts the computational effort from sampling in the full joint space of $(\bm{x},\bm{\theta})$ to first characterising the lower-dimensional marginal posterior $\pi(\bm{\theta}|\bm{y})$ and subsequently generating samples from the full joint posterior through the conditional distribution $\pi(\bm{x}|\bm{\theta},\bm{y})$. Furthermore, Fox and Norton}~\cite{fox2016fast}\mccorrect{ demonstrate that MTC sampling outperforms standard MCMC approaches for linear-Gaussian hierarchical inverse problems.}
...
\begin{align}
	...
	\label{eq:LinDat}
\end{align}
Then we can obtain the marginal and full conditional distribution explicitly.
...
For the derivation of the marginal posterior and the full conditional distribution, consider the joint multivariate Gaussian distribution
\begin{align}
	\begin{pmatrix}
		\bm{x} \\
		\bm{y}
...
Immediately\footnote{$\text{Assume }
		...
		\bm{x} |  \bm{y} \sim \mathcal{N}\big(	\bm{\mu}_{\bm{x}} -   \bm{Q}^{-1}_{\bm{x} \bm{x}} \bm{Q}_{\bm{x} \bm{y}}(\bm{y} - \bm{\mu}_{\bm{y}} ) , \bm{Q}^{-1}_{\bm{x} \bm{x}} \big).$}, the full conditional distribution can be formulated as
\begin{equation}
...
\begin{mccorrection}
Within this scheme, we evaluate the marginal posterior first and then condition on hyper-parameter samples from the marginal posterior to draw full conditional samples $\bm{x} \sim \pi (\bm{x} | \bm{\theta}, \bm{y})$ (see Sec.~\ref{subsec:RTO}).
Alternatively we can evaluate the posterior mean
\begin{align}
...
%\end{align}
\begin{align}
	\Sigma_{\bm{x}|\bm{y}} &= \mathbb{E}_{\bm{\theta|\bm{y}}} \left[\bm{\Sigma}_{\bm{x}|\bm{\theta} ,\bm{y}}\right] + \mathbb{E}_{\bm{\theta|\bm{y}}} \left[ (\bm{\mu}_{\bm{x}|\bm{\theta} ,\bm{y}} - \bm{\mu}_{\bm{x}|\bm{y}} ) (\bm{\mu}_{\bm{x}|\bm{\theta} ,\bm{y}} - \bm{\mu}_{\bm{x}|\bm{y}} )^T \right]   \\
	 &= \int \bm{\Sigma}_{\bm{x} | \bm{\theta} , \bm{y}} \pi(\bm{\theta}| \bm{y}) \diff \bm{\theta} + \int (\bm{\mu}_{\bm{x}|\bm{\theta} ,\bm{y}} - \bm{\mu}_{\bm{x}|\bm{y}} ) (\bm{\mu}_{\bm{x}|\bm{\theta} ,\bm{y}} - \bm{\mu}_{\bm{x}|\bm{y}} )^T  \pi(\bm{\theta}| \bm{y}) \diff \bm{\theta} \label{eq:CoVarGenInt} \, ,
\end{align}
which is given by the law of total covariance, of $\pi(\bm{x}| \bm{y})$ by some quadrature rule.
\end{mccorrection}
...
Within the MTC scheme we draw samples $\{  \bm{\theta}^{(1)}, \dots,  \bm{\theta}^{(k)}, \dots, \bm{\theta}^{(N)} \} \sim \pi(\bm{\theta} | \bm{y})$ from the marginal posterior first and then characterise the full conditional $\pi(\bm{x}|\bm{\theta} , \bm{y})$.
...
%, which in our case will be the marginal posterior distribution over the hyper-parameters,
...
\mccorrect{Non-negativity is important because the target function represents a probability density function.
One way to ensure non-negativity is to approximate the square root of the target function.
An alternative would be to approximate the log-density. While this may provide certain numerical advantages, it cannot be used directly within the sampling framework introduced later. Specifically, the proposed inverse Rosenblatt transform (IRT) scheme is computationally attractive because integration and marginalisation can be performed directly within the TT format, allowing samples to be generated efficiently once the function has been approximated in TT format. This property would be lost when approximating the log-density, since the resulting approximation no longer directly represents a (non-negative) density and therefore cannot be sampled from within the proposed IRT framework without additional steps.}

Hence, Cui and Dolgov~\cite{cui2022deep} approximate the square root of the target function and 
...
\begin{align}
	\sqrt{\pi(\bm{x})} \approx \tilde{g}(\bm{x}) = \bm{G}_1(x_1) \cdots \bm{G}_k(x_k) \cdots \bm{G}_d(x_d)\, .
\end{align}
...
\begin{align}
	\pi(\bm{x}) \approx \tilde{\pi}(\bm{x}) = \xi + \tilde{g}(\bm{x})^2 \label{eq:SQTTapprox}
\end{align}
...
\begin{equation}
	f_{X_k}(x_k)  \approx \frac{1}{z} \left(\xi \prod_{i=1}^{k-1} \uplambda_i(\mathcal{X}_i) \prod_{i=k+1}^{d} \uplambda_i(\mathcal{X}_i) + \sum_{l_{k-1}=1}^{r_{k-1}} \sum_{l_k=1}^{r_k} \left(\sum_{i=1}^{n} \phi^{(i)}_k(x_k) \bm{D}_k[l_{k-1},i, l_k] \right)^2 \right) \uplambda_k(x_k), \label{eq:MargTT}
\end{equation}
...
\end{align} %\textcolor{red}{Argghhhh!}
For $k = 2, \dots,d$ the ``conditional marginal'' is given as \cite[Eq.~31]{cui2022deep}:
\begin{align}\begin{split} 
...
\begin{algorithm}[!ht]
	...
	\begin{algorithmic}[1]
		\STATE \textbf{Input:} samples $\{ \bm{x}^{(1)},\dots, \bm{x}^{(N+1)} \}  \sim \tilde{\pi}(\bm{x})$, where each $\bm{x}^{(s)} \in \mathbb{R}^d$ for $s = 1, \dots, N+1$, and intialise $\bm{x}_{\text{MH}}^{(1)} = \bm{x}^{(1)}$.
		\FOR{ \( s = 1, \dots, N-1\)}
		\STATE Compute MH ratio $\frac{w^{(s+1)}}{w^{(s)}_{\text{MH}} } =\frac{\pi(\bm{x}^{(s+1)})}{\pi(\bm{x}_{\text{MH}}^{(s)})} \frac{f_X(\bm{x}_{\text{MH}}^{(s)})}{f_X(\bm{x}^{(s+1)})}$.
		\STATE Compute acceptance probability $\alpha = \text{min}\left(w^{(s+1)}/w_{\text{MH}}^{(s)}, 1\right)$. 
		\STATE Draw $u \sim \mathcal{U}(0,1)$.
		\IF{$\alpha \geq u$ }
		\STATE Accept and set $\bm{x}_{\text{MH}}^{(s+1)} = \bm{x}^{(s+1)}$.
		\ELSE  
		\STATE Reject and keep $\bm{x}_{\text{MH}}^{(s+1)} = \bm{x}_{\text{MH}}^{(s)}$.
		\ENDIF
		\ENDFOR
		\STATE \textbf{Output:} corrected sample chain $\{ \bm{x}_{\text{MH}}^{(1)},\dots, \bm{x}_{\text{MH}}^{(N)} \} \sim \pi(\bm{x}) $.
	\end{algorithmic}
	\label{alg:MHCorr}
\end{algorithm}


In doing so, we compute the acceptance probability $  \alpha = \text{min}(w^{(s+1)}/w^{(s)}_{\text{MH}}, 1)$, where 
\begin{align}
	w(x) = \frac{\pi(\bm{x})}{f_X(\bm{x})} = \frac{\pi(\bm{x})}{\xi + \tilde{g}(\bm{x})^2} 
\end{align}
is the importance ratio.
Note the normalising constants in the ratio $w^{(s+1)}/w^{(s)}_{\text{MH}}$ cancel.
In practice, the MH importance ratio is calculated in the log-space so that $\log f_X(\bm{x})  =  \log f_{X_1}(x_1) + \log f_{X_2|X_1}(x_2|x_1) + \cdots + \log f_{X_k|X_{<k}}(x_k|x_{k-1},\dots,x_1)$ (see Eq.~\ref{eq:CurrMarg}).
...
\begin{mccorrection}
The choice of approximating the square root of the target density is not solely motivated by numerical stability. In particular, the bound
\begin{align*} 
...
\end{align*}
guarantees uniform ergodicity of the corrected chain and establishes convergence of MH correction algorithm~\cite{cui2022deep}.
\end{mccorrection}
...
\mccorrect{Further, the Wasserstein distance measures the discrepancy between the distribution due to the TT approximation and the exact distribution.
As the TT approximation error decreases, the Wasserstein distance between the two distributions also decreases. 
This is relevant because, through the Kantorovich–Rubinstein duality, the Wasserstein distance relates to the error in expectations of 1-Lipschitz functions.
} 
...
For two sample sets $\{ \bm{x}^{(1)},\dots,\bm{x}^{(N)}\} \sim \pi$ and $\{\tilde{ \bm{x}}^{(1)},\dots,\tilde{\bm{x}}^{(M)}\} \sim \tilde{\pi}$ the calculation of the Wasserstein distance becomes an optimisation problem.
...
\end{align}
is a discrete approximation to the second-order derivative with Neumann boundary conditions~\cite{wang2015graphs}.
...
%~\cite{VowelsMultiD}.
%For a given data misfit norm, every sample of the high-dimensional posterior, which represents a feasible solution given the data, is less regularised and has a higher $ \bm{x}^T \bm{L} \bm{x}$ value.
