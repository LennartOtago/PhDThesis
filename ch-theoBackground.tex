...
We read $\pi(\bm{y}|\bm{\theta},\bm{x})$ as the distribution over $\bm{y}$ conditioned on $\bm{x}$ and $\bm{\theta}$.
...
If the integral in Eq.~\ref{eq:2fullCond} is expensive to calculate, we use sample based methods to produce a Markov chain $\{ (\bm{x}, \bm{\theta})^{(1)}, \dots, (\bm{x}, \bm{\theta})^{(N)} \} \sim \pi(\bm{x}, \bm{\theta} |  \bm{y}) $ and sample from $\pi(\bm{\theta} |  \bm{y})$ first and then draw samples from the conditional $\pi(\bm{x} | \bm{\theta} , \bm{y})$. 
...
Within this scheme, we evaluate the marginal posterior first and then either condition on hyper-parameters to draw conditional posterior samples $\bm{x} \sim \pi (\bm{x} | \bm{\theta}, \bm{y})$ (see Sec. \ref{subsec:RTO}) or evaluate the mean
\begin{align}
...
\label{sec:sampling}
In this section we present the methods that draw samples from the marginal posterior $ \mathcal{M} = \{  \bm{\theta}^{(1)}, \dots,  \bm{\theta}^{(k)}, \dots, \bm{\theta}^{(N)} \} \sim \pi(\bm{\theta} |  \bm{y})$ as well as the RTO method to draw samples from a normally distributed conditional posterior $\pi(\bm{x}|\bm{\theta} , \bm{y})$.
...
\begin{align}
	...
	\label{eq:LinPol}
\end{align}
for $x^{(i)}_k \leq x^{(s)}_k \leq x^{(i+1)}_k$ in between two grid points $i$ and $i+1$ as in~\cite{dolgov2020approximation} for the next ``conditional marginal''.
...
\begin{align}
	\Bigg( \frac{1}{N} \sum^{N}_{i =1} \Big(\pi(\bm{x}^{(i)}) - \big(\gamma + \tilde{g}(\bm{x}^{(i)})^2\big)\Big)^2 \uplambda(\bm{x}^{(i)})\Bigg)^{1/2}    \approx \Bigg(  \int_{\mathcal{X}} (\pi(\bm{x}) - \big(\gamma + \tilde{g}(\bm{x})^2\big)\Big)^2 \uplambda(\bm{x}) \diff \bm{x} \Bigg)^{1/2} \label{eq:RMSTT}
\end{align}
...
\label{subsec:wasser}
One way to assess the error between two distributions is to calculate the Wasserstein distance.
The Kantorovich-Rubinstein duality, as in~\cite{thickstun2019kantorovich, Ambrosio2024Kanta}, says that the 1-Wasserstein distance is equal to the upper bound of differences in expectations of a function $h$ between two probability distributions.
...
Instead, we approximate the non-linear model using an affine map $ \bm{M}:\bm{A}_L\bm{x} \rightarrow \bm{A}_{NL}(\bm{x})$, which maps from the linear model to the non-linear model, so that we define linear $ \bm{A} = \bm{M} \bm{A}_{L}$, which approximates the non-linear model.
