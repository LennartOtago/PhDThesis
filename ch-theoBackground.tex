...
This is maintains that the cumulative distribution functions (CDFs) are monotonically increasing.
The CDFs are needed to draw samples from $\tilde{\pi}(\bm{x})$ (see Sec.~\ref{subsec:SamplTT}).
...
Hence, every sample of the high-dimensional posterior, which represents a feasible solution given the data, lies above the L-Curve, is less regularised and has a higher $ \bm{x}^T \bm{L} \bm{x}$ value.
