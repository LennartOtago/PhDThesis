...
Asumme that the triple $(\Omega, \mathcal{F} , \mathbb{P})$ is called a probability space over the whole sample space $\Omega$ with the collection $\mathcal{F}$ of very countable subset $\{ A _n \}_{n\in \mathbb{N}}$, where a Event $A_n$ is a subset of $\Omega$, $A_n \subseteq  \Omega$, and a map $\mathbb{P} : \mathcal{F} \longrightarrow \mathbb{R}$. 
...
\end{align}
using the Radon-Nikodym theroem \cite{}
\begin{align}
	\mathbb{P}(A) = \int_{A}  \frac{\diff \mathbb{P}}{\diff x} \diff x
\end{align}
where $\diff x$ is a measure on the same probality space and we call this the lebesgue measure and $\frac{\diff \mathbb{P}}{\diff x}$ is often called Radon-Nikodym derivativ, which can be set to the probaility density $ \pi(x)$ of a random variable $X: \Omega \longrightarrow \mathbb{R}$.
Next we intrdouce a weight function $\uplambda(x) $, which can be helpful for qraudrature rules, so that
\begin{align}
...
For the caclultion of marginal PDF, asuume the random variabes $(X_1 , \dots, X_d): \Omega \longrightarrow \mathbb{R}^d $ define a joint distributiont trought the probailty density  function $\pi(x)$ then the marginal distribution is 
\begin{align}
		f_{X_k}(x_k) = \int_{\mathbb{R}} \cdots \int_{\mathbb{R}} \uplambda(x) \pi(x) \diff x_1 \cdots \diff x_{k-1} \diff x_{k+1} \cdots \diff x_d
\end{align}
...
\begin{equation}
	f_{X_k}(x_k) = \frac{1}{z} \left(\gamma^{\prime} \prod_{i=1}^{k-1} \uplambda_i(X_i) \prod_{i=k+1}^{d} \uplambda_i(X_i) + \sum_{l_{k-1}=1}^{r_{k-1}} \sum_{l_k=1}^{r_k} \left(\sum_{i=1}^{n_k} \phi^{(i)}_k(x_k) \bm{D}_k[l_{k-1},i, l_k] \right)^2 \right) \uplambda_k(x_k),
\end{equation}
