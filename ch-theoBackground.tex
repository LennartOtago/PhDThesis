...
The expectation of any function $h(\bm{x}_{\bm{\theta}})$, where $\bm{x}_{\bm{\theta}} \coloneqq \bm{x}(\bm{\theta})$ may depend on $\bm{\theta}$, is described as 
\begin{align}
...
\end{align}
over the hyper-parameters $\bm{\theta}$ (see~\cite[Lemma 2]{fox2016fast}).
...
\begin{align}
	\bm{x} | \bm{\theta} , \bm{y}\sim \mathcal{N}(\bm{\mu} + (\bm{Q}(\bm{\theta}) + \bm{A}^T \bm{\Sigma}(\bm{\theta})^{-1} \bm{A})^{-1}(\bm{y} - \bm{A}\bm{\mu}),(\bm{Q}(\bm{\theta}) + \bm{A}^T \bm{\Sigma}(\bm{\theta})^{-1} \bm{A})^{-1}) \, \label{eq:CondPostLin}.
\end{align}
...
If a parameter $\bm{x}$ is  functionally dependent on the hyper-parameters $\bm{\theta}$, i.e., $\bm{x} = \bm{x}(\bm{\theta})$, so that the number of hyper-parameters is larger than e.g. 2, we sample from the marginal posterior $\pi(\bm{\theta} | \bm{y})$ using the \texttt{t-walk} sampler as by Christen and Fox~\cite{christen2010general}. 
The \textbf{t-walk} is employed as a black-box algorithm, requiring the specification of the number of samples, burn-in period, support region, and the target distribution. 
Convergence to the target distribution is guaranteed by construction of this algorithm.


\subsection{Randomise then Optimise}
\label{subsec:RTO}
If we can not evaluate the mean and variance of the full posterior through integration, e.g. if it is computationally not feasible to solve the integral in Eq. \ref{eq:fullCond} due to a large number of hyper-parameters, we need an alternative way to characterise $\pi(\bm{x} |  \bm{\theta}, \bm{y})$.
