...
\textcolor{red}{If a mid-level or high-level model of $\bm{x}$ is available, we can represent the parameter space of $\mathbb{R}^n$ with a number of hyper-parameters much smaller than $n$~\cite{fox2016fast, Watzenig_2009}.
...
\begin{figure}[ht!]
	...
	\caption[]{This Figure not to scale is directly inspired by~\cite{ColinPhD} and shows how a solution $ \bm{x}_{\lambda}  = \underset{\bm{x}}{\text{arg min }}\bm{x}^T \bm{L} \bm{x}$ subject to the constraint $\left\lVert \bm{y} - \bm{A} \bm{x} \right\rVert^2_{L^2}= c$ is obtained.}
