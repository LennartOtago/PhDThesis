...
So every solution $\bm{x}_{\lambda}$ is extremely regularised for a given data misfit and the L-Curve presents the lower boundary to an open set of $\bm{x}^T \bm{L} \bm{x}$ values.
Hence, almost every sample of the posterior, which represents a feasible solution given the data, lies above the L-Curve and is less regularised and because it has a higher $ \bm{x}^T \bm{L} \bm{x}$ value.
