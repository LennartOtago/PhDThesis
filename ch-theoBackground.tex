...
The central limit theorem states that the sample mean $ \bm{\mu}^{(i)}_{\text{samp}} = \text{E}_{\bm{x},\bm{\theta}|\bm{y}} [h(\bm{x})]$ of independent sample sets $\mathcal{M}_i$ for $i = 1, \dots, n$ of any distribution, converges to be normally distributed, so that
\begin{align}
...
The estimation error $(\sigma^{(i)})^2 =  \text{var}(\text{E}_{\bm{x},\bm{\theta}|\bm{y}} [h(\bm{x})]) $ can be approximated by:
\begin{align}
...
\end{align}
Here $\rho(t) \longrightarrow 0$ for $t \rightarrow \infty$ is the normalised autoccoraltino coeffciten at lag $t$ and  $\tau_{\text{int}}$ is the integrated autocorrelation time (IACT) as in \cite[pp. 103-105]{wolff2002LecNot}.
We have to consider the IACT within our error estimate since samples in a sample set generated by any system or algorithm are correlated.
...
Since the IATC is an estimate itself \cite{wolff2004monte} provides a way to not only calculate the IACT but also quantifies the errors of the estimated IACT.
We point out that for uncorrelated smaples $2\tau_{\text{int}} = 1$ and the error $(\sigma^{(i)})^2$ is a typical Monte-Carlo estimate.
See Appendix \ref{ap:IATC} and \cite{Sokal1997, wolff2002LecNot} for a more detailed derivation.
...
In other words, the chain can reach any state from any other state (irreducibility), is not stuck in periodic cycles (aperiodicity), and satisfes the detailed balance condition~\cite{tan2016LecNot} (reversibility).
...
The \texttt{t-walk} is employed as a black-box algorithm in default settings, requiring the specification of the number of samples, burn-in period, support region, and the target distribution. 
