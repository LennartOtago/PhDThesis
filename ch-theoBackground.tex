...
%We calculate the IACT using the Python implementation of \cite{wolff2004monte}, accessed \cite{wolff2004monte}, which also provides errors on the IACT and helps to assess convergence of the chain.
...
\subsubsection{realtive root mean sqaured error}
...
\begin{align}
	W_1(\pi,\tilde{\pi}) = \underset{  \nu \in \Pi(\pi,\tilde{\pi}) }{ \text{inf}}\int_{\mathcal{X} \times \mathcal{X}} c(\bm{x},\bm{y}) \, \nu(\bm{x},\bm{y}) d\bm{x} d\bm{y} 
	\label{eq:wass} \, ,
\end{align}
where $\nu$ couples $\bm{x}$ and $\bm{y}$ so that the intgral over the distance $ c(\bm{x},\bm{y}) $ weighted by the probility measures $\pi$ and $\tilde{\pi}$ is the greatest lower bound of all integral with repesct to the coupling $\nu$ in the set of all couplings $ \Pi(\pi,\tilde{\pi})$.
...
\begin{align}
	W_1(\pi,\tilde{\pi})  =& \frac{1}{K}\underset{ \lVert h(\bm{x})- h(\bm{y}) \rVert_{L^2} \leq   \lVert \bm{x} - \bm{y}  \rVert_{L^2} }{ \text{sup}} \Bigg\{  \int_{\mathcal{X}} h(\bm{x}) d \pi (\bm{x})  - \int_{\mathcal{X}} h(\bm{y}) d \tilde{\pi} (\bm{y}) \Bigg\} \\
		=& \underset{ \lVert h(\bm{x})- h(\bm{y}) \rVert_{L^2} \leq \lVert \bm{x} - \bm{y}  \rVert_{L^2} }{ \text{sup}}  \Bigg\{  \underset{\bm{x} \sim  \pi }{\mathbb{E}} \big[ h(\bm{x}) \big]  -  \underset{\bm{y} \sim \tilde{\pi}}{\mathbb{E}} \big[ h(\bm{y}) \big] \Bigg\} .
\end{align}
as the lowest upper bound of differences in expecations of the 1-Lipschitz function $h$ over the two distributions $\pi$ and $\tilde{\pi}$, with distance measure $c(\bm{x},\bm{y})= \lVert \bm{x} - \bm{y} \rVert_{L^2} $ for $\mathcal{X} \in \mathbb{R}^d$.
For two sample sets $\{ \bm{x}^{(1)},\dots,\bm{x}^{(1)}\} \sim \pi$ and $\{ \bm{y}^{(1)},\dots,\bm{y}^{(1)}\} \sim \tilde{\pi}$ the caculation of the Wasserstein distance becomes an optimasation problem, that is to find the best permutation (order) of samples weighted by their distribution value acoording to an approiate diastnce measure \cite{feydy2020OT}.
...
\begin{align}
	W_2(\pi,\tilde{\pi}) = \underset{  \nu \in \Pi(\pi,\tilde{\pi}) }{ \text{inf}}  \Bigg(\int_{\mathcal{X} \times \mathcal{X}} c(\bm{x},\bm{y})^2 \, \nu(\bm{x},\bm{y}) d\bm{x} d\bm{y} \Bigg)^{1/2}
	\label{eq:wass} \, .
\end{align}
