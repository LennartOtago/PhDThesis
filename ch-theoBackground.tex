...
If $\bm{x}$ cannot be parametrised directly in terms of the hyper-parameters $\bm{\theta}$, and only a low level representation of $\bm{x}$ is availiable it is beneficial to factorise the posterior distribution as
\begin{align}
...
This approach, known as the MTC method, is particularly advantageous when $\bm{x}\in \mathbb{R}^n$ is high-dimensional, while $\bm{\theta}$ is low-dimensional and the evaluation of $\pi(\bm{\theta}| \bm{y})$ is relatively cheap.
If a high level model of $\bm{x}$ availiabe we can represnt the paramters space of $\mathbb{R}^n$ with a number of hyper-parameters much smaller than $n$.
That is when Bayesian modelling is especially powerfull because we can impose prior knowlede throught those hyper-parameters much more precisely and the dimesnionalty of the posterior may be drastically reduced.
The marginal posterior over the hyper-parameter $\bm{\theta}$, that can be calculated by,
\begin{align}
...
We use Markov chain Monte-Carlo (MCMC) methods to calculate sample-based estimates of $\mathbb{E}_{\bm{x} ,\bm{\theta}  |\bm{y}} [h(\bm{x})] $ as in Eq.~\ref{eq:sampMean}.
Within the MTC scheme one may draw samples $\{  \bm{\theta}^{(1)}, \dots,  \bm{\theta}^{(k)}, \dots, \bm{\theta}^{(N)} \} \sim \pi(\bm{\theta} | \bm{y})$ from the marginal posterior first and then evaluate the full conditional posterior $\pi(\bm{x}|\bm{\theta} , \bm{y})$.
...
For large enough $N$ this chain of random variables can be used to calculate Monte-Carlo estimates, where ergodicity of $\mathcal{M}$ is a sufficient criterion to do so~\cite{tan2016LecNot, roberts2004general}.
...
Instead of relying on sampling-based methods to explore a target function $\pi(\bm{x})$, we can approximate this function on a $d$-dimensional grid using a tensor-train (TT) approximation $\tilde{\pi}(\bm{x}) \approx \pi(\bm{x})$, where $\bm{x} \in \mathbb{R}^d$, with far fewer function evaluation compared to conventional sampling methods.
In the following, we describe how to compute marginal functions from a probability density approximated in TT format and how to generate samples using a scheme similar to the inverse Rosenblatt transform (IRT) as in~\cite{dolgov2020approximation}.
In this section we follow the notation and procedure introduced in~\cite{cui2022deep}.
...
To approximate a target function in the TT format one has to predefined $d$-dimensional discrete univariate grid over the parameter space $\mathcal{X}$ with $n$ grid points in each direction.
Then the integral in Eq.~\ref{eq:margInt} for the marginal function can be computed at a low computational cost, as $\pi(\bm{x})$ is approximated by
\begin{align*}
...
A TT has outer ranks $r_0 = r_d = 1$.
...
In particular when non-negative functions (probability distributions) are approximated the TT approximation can have negative values in regions where true function values are very small.
...
Here, as in each TT core is given by \cite[Eq. 21]{cui2022deep}
\begin{equation}
...
The approximated unnormalised function (probability density) is written as \cite[Eq. 19]{cui2022deep}:
\begin{align}
...
This leads to the normalised target function \cite[Eq. 19]{cui2022deep}
\begin{align}
...
\label{subsec:TTMarg}
The marginal functions $f_{X_k}(x_k)$ of the target function $f_{X}(\bm{x})$ are computed by a procedure to which Cui et al.~\cite{cui2022deep} refer to as backwards marginalisation, see Prop.~\ref{prob:ForMarg}, and to which we add the forward marginalisation, see Prop.~\ref{prob:backMarg}. 
...
Instead of evaluating marginal functions for quadrature, the inverse Rosenblatt transform (IRT) provide a scheme to draw samples from an approximated function in the TT format~\cite{dolgov2020approximation}.
...
\begin{algorithm}[!ht]
	...
	\label{alg:MHCorr}
\end{algorithm}
In doing so, compute the acceptance probability $  \alpha = \text{min}(w^{(s+1)}/w^{(s)}, 1)$, where 
\begin{align}
