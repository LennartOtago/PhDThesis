...
\begin{align}
	...
	\label{eq:LinDat}
\end{align}
and we can derive the marginal and full conditional posterior distribution explicitly.
...
The full conditional distribution, as in Eq.~\ref{eq:CondPostLin}, can be rewritten as
\begin{align}
...
Instead, we approximate the non-linear model using an affine map $ \bm{M}:\bm{A}_L\bm{x} \rightarrow \bm{A}(\bm{x})$, which maps from the linear model $\bm{A}_{L}$ to the non-linear model $\bm{A}(\bm{x})$, so that we define a linear model $ \bm{A} \coloneqq \bm{M} \bm{A}_{L}$, which approximates the non-linear model.
