...
\end{align}
where the affine map $\bm{M}$ describes the mapping in bewtween the linear and the non-lineaer model for $\bm{x} = \bm{\mu}_{\bm{x}|\bm{y}} + \Delta \bm{x}$  around the prevously caclulated posterior mean $\bm{\mu}_{\bm{x}|\bm{y}} $ (see Fig.~\ref{fig:affinStrat} for the summarised strategy).
...
For  $\bm{x} = \bm{\mu}_{\bm{x}|\bm{y}} + \Delta \bm{x}$ with an affine map $\bm{A}\bm{x} + \bm{b}$ which is a linear map $\bm{A}$ with a constant $\bm{b}$.
\begin{align}
	...
	=    \underbrace{ \bm{A}^{\prime}( \bm{\mu}_{\bm{x}|\bm{y}} ) \bm{x}}_{ \coloneqq \bm{A}\bm{x}}  +  \underbrace{ \bm{A}( \bm{\mu}_{\bm{x}|\bm{y}} )  - \bm{A}^{\prime}( \bm{\mu}_{\bm{x}|\bm{y}} ) \bm{\mu}_{\bm{x}|\bm{y}}}_{ \coloneqq \bm{b}}
\end{align}
\begin{figure}[htb!]
	\centering
	\begin{tikzpicture}
...
The subspace $\bm{W}$ is created by noise-free data based on the linear model and $\bm{V}$ by noise-free data based on the non-linear model, given $m-1$ samples $\bm{x}^{(j)} \sim \pi(\bm{x}|\bm{y})$ for $j = 2, \dots,m$.
...
\section{Marginal and Then Conditional Posterior of Ozone}
