...
For parameters $\bm{x}$ around the posterior mean $\bm{\mu}_{\bm{x}|\bm{y}}$ the affine map $\bm{M}$ provides an approximation to the non-linear model $\bm{A}(\bm{x})$ based on the linear model $\bm{A}_L$.
